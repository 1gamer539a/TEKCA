...
const CATEGORIES = [
  "Recharge de jeu", "Accessoire", "Vêtement", "Abonnement", "Ebook", "Template",
...
const CATEGORIES_PHOTOS_STRICTES = ["Immobilier", "Véhicules", "Meuble & Déco"];
...
// Vêtement) restent protégées par le séquestre TEKÇA classique.
const CATEGORIES_COFFRE_FORT = ["Recharge de jeu", "Abonnement", "Ebook", "Template"];
...
export default function FormulaireProduit() {
  ...
  useEffect(() => {
    if (!vendeurId || categorie !== "Recharge de jeu" || !nom.trim()) { setSoldeJeu(0); return; }
  ...
  const rechargerWalletJeu = async () => {
    ...
    try {
      ...
      if (!reponse.ok) throw new Error(data.error || "Échec de la recharge.");
      setSoldeJeu(data.solde);
      setMontantRecharge("");
    } catch (e) {
      setErreurRecharge(e.message || "Échec de la recharge.");
    } finally {
  ...
  // maintenant autorisés dans les 19 pays SebPay, plus seulement CG.
  ...
  const publier = async () => {
    ...
    if (CATEGORIES_PHOTOS_STRICTES.includes(categorie) && photos.length < PHOTOS_MIN_STRICT) {
      setErreur(`Pour la catégorie ${categorie}, veuillez importer entre ${PHOTOS_MIN_STRICT} et ${PHOTOS_MAX_STRICT} photos afin de garantir la qualité de l'annonce.`);
      return;
    }
    ...
    try {
      ...
      // a été retirée : Le Marché est ouvert aux 19 pays SebPay)
    ...
    return (
      <div style={{ background: COLORS.background, minHeight: "100vh", color: COLORS.textPrimary }} className="flex flex-col items-center justify-center px-6 text-center">
        ...
        <p className="text-sm mt-2" style={{ color: COLORS.textMuted }}>
          {produitCreeId
            ? "Aucun code de l'app n'a été touché. Il te reste une étape avant de pouvoir vendre : configure ton Coffre-fort (au moins un code en stock) — sans ça, ce produit ne pourra recevoir aucune commande."
    ...
    <div style={{ background: COLORS.background, minHeight: "100vh", color: COLORS.textPrimary }}>
      ...
      <main className="max-w-md mx-auto w-full px-4 pt-20 pb-10 flex flex-col gap-4">
        ...
        <div>
          ...
          {CATEGORIES_COFFRE_FORT.includes(categorie) && (
            <p className="text-[11px] mt-2 rounded-lg px-3 py-2" style={{ background: COLORS.surface, color: COLORS.accentSecondary, border: `1px solid ${COLORS.border}` }}>
              🔒 Ce produit est livré via le Coffre-fort numérique : paiement immédiat, pas de séquestre. Tu devras y ajouter au moins un code avant de pouvoir recevoir une commande.
            </p>
          )}
        </div>
        ...
        <div className="rounded-2xl p-4 flex flex-col gap-3" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
          <label className="text-sm font-semibold">Comment ce produit est-il remis ?</label>
          <div className="flex flex-wrap gap-2">
            {[
              { valeur: "domicile", label: "📦 Livraison à domicile" },
              { valeur: "main_propre", label: "🤝 Remise en main propre" },
              { valeur: "instantane", label: "⚡ Livraison instantanée / Digitale" },
            ].map((m) => {
          ...
          {(modesRemise.includes("domicile") || modesRemise.includes("main_propre")) && (
            <div>
              <label className="text-xs font-semibold block mb-1.5" style={{ color: COLORS.textMuted }}>
                Zone de couverture (quartiers/villes concernés)
              </label>
              <input
                value={zoneCouverture}
                onChange={(e) => setZoneCouverture(e.target.value)}
                placeholder="Ex: Bacongo, Poto-Poto, Talangaï"
        ...
        <div className="rounded-2xl p-4" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
          <label className="text-xs font-semibold block mb-2">
            ...
            {CATEGORIES_PHOTOS_STRICTES.includes(categorie) && (
              <span className="font-normal" style={{ color: COLORS.textMuted }}> — entre {PHOTOS_MIN_STRICT} et {PHOTOS_MAX_STRICT} obligatoires</span>
            )}
          </label>
        ...
        {categorie === "Accessoire" && (
          <div className="rounded-2xl p-4" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
            <label className="text-xs font-semibold block mb-1">Quantité disponible en stock</label>
            <input
              value={stockGlobal}
            ...
            <p className="text-[10.5px] mt-1" style={{ color: COLORS.textMuted }}>
              Le produit ne pourra plus être commandé une fois ce stock épuisé.
            </p>
          </div>
        )}
          ...
          <div className="rounded-2xl p-4" style={{ background: COLORS.surface, border: `1px solid ${COLORS.accentPrimary}` }}>
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                style={{ background: COLORS.accentPrimary, color: COLORS.background }}
              >
                {rechargeEnCours ? "..." : "Recharger"}
              </button>
            </div>
            {erreurRecharge && <p className="text-[11px] mt-2" style={{ color: "#B23A2E" }}>{erreurRecharge}</p>}
            {!nom.trim() && <p className="text-[11px] mt-2" style={{ color: COLORS.textMuted }}>Donne d'abord un nom au produit (le jeu concerné) pour pouvoir recharger.</p>}
          </div>
        )}
