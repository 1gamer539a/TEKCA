...
export default function FilDiscussion() {
  ...
  const [nomVendeur, setNomVendeur] = useState(t("discussion.conversationDefaut"));
  ...
  useEffect(() => {
    ...
    const charger = async () => {
      ...
      const { data: conv } = await supabase
        .from("conversations")
        .select("id, client_id, vendeurs ( user_id, nom_boutique ), users:client_id ( nom )")
        .eq("id", conversationId)
        .single();
      if (conv) {
        // CORRECTIF — affichait toujours le nom de la boutique, même
        // quand c'est LE VENDEUR LUI-MÊME qui ouvre ce fil depuis son
        // dashboard : il voyait le nom de sa propre boutique au lieu
        // du nom du client avec qui il discute. On affiche maintenant
        // le nom de l'AUTRE participant, selon qui regarde.
        const estLeVendeur = conv.vendeurs?.user_id === user.id;
        setNomVendeur(estLeVendeur ? (conv.users?.nom || t("discussion.client")) : (conv.vendeurs?.nom_boutique || t("messages.vendeurDefaut")));
      }
      ...
      if (msgs) {
        ...
        // CORRECTIF — le badge "non lus" (DashboardVendeur.jsx) ne
        // redescendait jamais à zéro : rien ne marquait jamais un
        // message comme lu, même après l'avoir ouvert et affiché ici.
        const idsNonLus = msgs.filter((m) => m.expediteur_id !== user.id).map((m) => m.id);
        if (idsNonLus.length > 0) {
          supabase.from("messages_chat").update({ lu: true }).in("id", idsNonLus).then(() => {});
        }
      }
      setChargement(false);

      // CORRECTIF — cet abonnement temps réel était mis en place plus
      // haut dans l'effet, AVANT que "charger()" (async) n'ait fini
      // de résoudre user.id. Le callback ci-dessous fermait donc sur
      // la valeur de userId au moment du montage — toujours `null` à
      // vie pour cet abonnement — et classait donc TOUS les messages
      // reçus en temps réel comme venant de "l'autre", y compris les
      // tiens : ton propre message envoyé n'apparaissait jamais du
      // bon côté (ou pas du tout si le temps réel arrivait avant
      // l'ajout optimiste dans envoyer()). On utilise ici la variable
      // locale user.id, connue à coup sûr à ce stade, au lieu de la
      // state React capturée par la closure.
          ...
          (payload) => {
            setMessages((prev) => {
              ...
              return [
                ...
                {
                  id: payload.new.id,
                  role: payload.new.expediteur_id === user.id ? "moi" : "autre",
    ...
    let canalActif = null;
    charger().then((canal) => { canalActif = canal; });

    return () => {
      if (canalActif) supabase.removeChannel(canalActif);
    };
  }, [conversationId]);
  ...
  const envoyer = async () => {
    ...
    if (!reponse.ok) {
      ...
      setAvertissement(data.message || t("discussion.messageNonEnvoye"));
      return;
    }
  ...
  return (
    <div style={{ background: COLORS.background, minHeight: "100vh", color: COLORS.textPrimary }} className="flex flex-col">
      ...
      <main className="flex-1 max-w-md mx-auto w-full px-4 pt-20 pb-24 flex flex-col gap-3">
        {chargement && (
          <p className="text-sm text-center py-10" style={{ color: COLORS.textMuted }}>{t("discussion.chargementMessages")}</p>
        )}
        {!chargement && messages.length === 0 && (
          <p className="text-sm text-center py-10" style={{ color: COLORS.textMuted }}>{t("discussion.disBonjour")}</p>
        )}
        {messages.map((m) => (
            ...
            <div className="max-w-[80%]">
              ...
              {m.ia && (
                <span className="text-[10px] flex items-center gap-1 mt-1" style={{ color: COLORS.accentSecondary }}>
                  <Sparkles size={10} /> {t("discussion.reponseIA")}
                </span>
              )}
            </div>
          </div>
      ...
      >
        <div className="max-w-md mx-auto flex items-center gap-2 rounded-xl px-3 py-2" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
          <input
            ...
            placeholder={compteBloque ? t("discussion.compteBloqueMsg") : t("discussion.ecrireMessage")}
            disabled={compteBloque}
            className="flex-1 bg-transparent outline-none text-sm"
            style={{ color: COLORS.textPrimary }}
          />
          <button onClick={envoyer} disabled={compteBloque} aria-label={t("discussion.envoyer")}><Send size={18} color={compteBloque ? COLORS.textMuted : COLORS.accentPrimary} /></button>
        </div>
      </div>
    </div>
  );
