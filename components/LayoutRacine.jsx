...
import EnqueteAttribution from "./EnqueteAttribution";
import BoutonFeedbackFlottant from "./BoutonFeedbackFlottant";
import NotificationsTempsReel from "./NotificationsTempsReel";
import InvitationInstallationPWA from "./InvitationInstallationPWA";
...
export default function LayoutRacine({ children }) {
  ...
  // CORRECTIF — ces deux composants étaient figés sur THEMES.sombre
  // et theme="sombre" en dur, sans jamais lire le vrai choix de
  // l'utilisateur (voir ThemeContext) : la nav du bas et le bouton IA
  // restaient sombres même en mode clair.
  ...
  return (
    <div className="relative">
      ...
      <NavigationBas theme={theme} />

      <EnqueteAttribution theme={theme} />
      <InvitationInstallationPWA theme={theme} />
      <NotificationsTempsReel />

      {!iaOuverte && (
        <>
          <BoutonIAFlottant COLORS={COLORS} onOpen={() => setIaOuverte(true)} />
          <BoutonFeedbackFlottant COLORS={COLORS} />
        </>
      )}
