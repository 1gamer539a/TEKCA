...
const LABELS_CANAL_ATTRIBUTION = {
...
const LABELS_TYPE_FEEDBACK = { suggestion: "Suggestion", amelioration: "Amélioration", bug: "Bug", autre: "Autre" };
const LABELS_PRIORITE_FEEDBACK = { faible: "Faible", moyen: "Moyen", eleve: "Élevé" };
const COULEURS_PRIORITE_FEEDBACK = { faible: "#8B96AD", moyen: "#C99A3A", eleve: "#B23A2E" };
...
export default function SalleSurveillance() {
  ...
  // Onglet Attributions ("comment as-tu connu TEKÇA")
  const [attributions, setAttributions] = useState([]);
  const [chargementAttributions, setChargementAttributions] = useState(false);

  // Onglet Feedbacks
  const [feedbacks, setFeedbacks] = useState([]);
  const [chargementFeedbacks, setChargementFeedbacks] = useState(false);
  const [filtreFeedback, setFiltreFeedback] = useState("tous");
  ...
  useEffect(() => {
    ...
    const charger = async () => {
      ...
      // Feedbacks chargés ici (pas en lazy comme "utilisateurs") pour
      // que le badge "Avis (N)" dans les onglets soit juste dès l'ouverture
      // du panneau, au même titre que les badges Signalements/KYC/Produits.
      chargerFeedbacks();
  ...
  const chargerAttributions = async () => {
    setChargementAttributions(true);
    const { data } = await supabase
      .from("user_attributions")
      .select("id, channel, details, created_at, users:user_id ( nom, email )")
      .order("created_at", { ascending: false })
      .limit(200);
    setAttributions(
      (data || []).map((a) => ({
        id: a.id,
        utilisateur: a.users?.nom || a.users?.email || "Utilisateur",
        canal: a.channel,
        details: a.details,
        temps: new Date(a.created_at).toLocaleDateString("fr-FR"),
      }))
    );
    setChargementAttributions(false);
  };

  const chargerFeedbacks = async () => {
    setChargementFeedbacks(true);
    const { data } = await supabase
      .from("user_feedbacks")
      .select("id, type, priority, message, status, created_at, users:user_id ( nom, email )")
      .order("created_at", { ascending: false })
      .limit(200);
    setFeedbacks(
      (data || []).map((f) => ({
        id: f.id,
        utilisateur: f.users?.nom || f.users?.email || "Utilisateur",
        type: f.type,
        priorite: f.priority,
        message: f.message,
        statut: f.status,
        temps: new Date(f.created_at).toLocaleDateString("fr-FR"),
      }))
    );
    setChargementFeedbacks(false);
  };

  const changerStatutFeedback = async (id, statut) => {
    setFeedbacks((prev) => prev.map((f) => (f.id === id ? { ...f, statut } : f)));
    await supabase.from("user_feedbacks").update({ status: statut }).eq("id", id);
  };
    ...
    <div style={{ background: COLORS.background, minHeight: "100vh", color: COLORS.textPrimary }}>
      ...
      <div className="fixed top-14 left-0 right-0 z-30 flex overflow-x-auto" style={{ background: COLORS.background, borderBottom: `1px solid ${COLORS.border}` }}>
        {[
          ...
          { id: "feedbacks", label: `Avis (${feedbacks.filter((f) => f.statut === "nouveau").length})` },
        ].map((o) => (
          <button
            key={o.id}
            onClick={() => {
              ...
              if (o.id === "attributions" && attributions.length === 0) chargerAttributions();
              if (o.id === "feedbacks" && feedbacks.length === 0) chargerFeedbacks();
            }}
      ...
      <main className="max-w-md mx-auto w-full px-4 pt-28 pb-10 flex flex-col gap-3">
        ...
        {/* ONGLET ATTRIBUTIONS — enquête "comment as-tu connu TEKÇA" */}
        {onglet === "attributions" && (
          <>
            {chargementAttributions && <p className="text-xs text-center" style={{ color: COLORS.textMuted }}>Chargement...</p>}
            {!chargementAttributions && attributions.length === 0 && (
              <p className="text-xs text-center py-6" style={{ color: COLORS.textMuted }}>Aucune réponse pour le moment.</p>
            )}
            {attributions.map((a) => (
              <div key={a.id} className="rounded-xl p-3" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
                <div className="flex items-center justify-between">
                  <p className="text-xs font-semibold">{a.utilisateur}</p>
                  <span className="text-[10px]" style={{ color: COLORS.textMuted }}>{a.temps}</span>
                </div>
                <p className="text-xs mt-1" style={{ color: COLORS.accentSecondary }}>{LABELS_CANAL_ATTRIBUTION[a.canal] || a.canal}</p>
                {a.details && <p className="text-xs mt-1" style={{ color: COLORS.textMuted }}>{a.details}</p>}
              </div>
            ))}
          </>
        )}

        {/* ONGLET FEEDBACKS — retours et suggestions utilisateurs */}
        {onglet === "feedbacks" && (
          <>
            <div className="flex gap-2 overflow-x-auto pb-1">
              {[
                ...
                { id: "amelioration", label: "Améliorations" },
                { id: "bug", label: "Bugs" },
              ].map((f) => (
                <button
                  key={f.id}
                  onClick={() => setFiltreFeedback(f.id)}
                  className="flex-shrink-0 rounded-full px-3 py-1.5 text-[10px] font-semibold"
                  style={{
                    background: filtreFeedback === f.id ? COLORS.accentPrimary : COLORS.surface,
                    color: filtreFeedback === f.id ? COLORS.background : COLORS.textMuted,
                    border: `1px solid ${COLORS.border}`,
                  }}
                >
                  {f.label}
            ...
            {chargementFeedbacks && <p className="text-xs text-center" style={{ color: COLORS.textMuted }}>Chargement...</p>}
            {!chargementFeedbacks && feedbacks.filter((f) => filtreFeedback === "tous" || f.type === filtreFeedback).length === 0 && (
              <p className="text-xs text-center py-6" style={{ color: COLORS.textMuted }}>Aucun avis pour le moment.</p>
            )}
            {feedbacks
              .filter((f) => filtreFeedback === "tous" || f.type === filtreFeedback)
              .map((f) => (
                <div key={f.id} className="rounded-xl p-3" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
                  <div className="flex items-center justify-between mb-1">
                    <p className="text-xs font-semibold">{f.utilisateur}</p>
                    <span className="text-[10px]" style={{ color: COLORS.textMuted }}>{f.temps}</span>
                  </div>
                  <div className="flex items-center gap-2 mb-1.5">
                    <span className="text-[10px] px-2 py-0.5 rounded-full font-semibold" style={{ background: COLORS.background, color: COLORS.accentSecondary, border: `1px solid ${COLORS.border}` }}>
                      {LABELS_TYPE_FEEDBACK[f.type] || f.type}
                    </span>
                    <span className="text-[10px] px-2 py-0.5 rounded-full font-semibold" style={{ background: COLORS.background, color: COULEURS_PRIORITE_FEEDBACK[f.priorite], border: `1px solid ${COLORS.border}` }}>
                      {LABELS_PRIORITE_FEEDBACK[f.priorite] || f.priorite}
                    </span>
                  </div>
                  <p className="text-xs mb-2" style={{ color: COLORS.textMuted }}>{f.message}</p>
                  <div className="flex gap-2">
                    {[
                      ...
                      <button
                        key={s.id}
                        onClick={() => changerStatutFeedback(f.id, s.id)}
                        className="text-[9px] px-2 py-1 rounded-full"
                        style={{
                          background: f.statut === s.id ? COLORS.accentPrimary : "transparent",
                          color: f.statut === s.id ? COLORS.background : COLORS.textMuted,
                          border: `1px solid ${COLORS.border}`,
                        }}
                      >
                        {s.label}
