  ...
  return (
    <div style={{ background: COLORS.background, minHeight: "100vh", color: COLORS.textPrimary }}>
      ...
      >
        ...
        <span className="text-sm font-semibold flex items-center gap-2"><Zap size={16} color={COLORS.accentPrimary} /> {t("divers.ventesFlashTitre")}</span>
      </header>
      ...
      <main className="pt-20 pb-16 px-4 max-w-md mx-auto w-full">
        <div className="rounded-xl p-3 mb-4 flex items-center gap-2 text-xs" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
          ...
          <span style={{ color: COLORS.textMuted }}>{t("divers.premiumVoientAvant")}</span>
        </div>

        {chargement && <p className="text-sm text-center" style={{ color: COLORS.textMuted }}>{t("common.chargement")}</p>}
        {!chargement && ventes.length === 0 && (
          <p className="text-sm text-center" style={{ color: COLORS.textMuted }}>{t("divers.aucuneVenteFlash")}</p>
        )}
              ...
              <div key={v.id} className="rounded-xl p-4 flex items-center justify-between" style={{ background: COLORS.surface, border: `1px solid ${COLORS.accentPrimary}` }}>
                <div>
                  ...
                  {pasEncoreVisiblePourTous && (
                    <p className="text-[10px] mt-1" style={{ color: COLORS.accentSecondary }}>{t("divers.accesAnticipePremium")}</p>
                  )}
                </div>
