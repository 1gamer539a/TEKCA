...
const MENU_SECTIONS = [
  { labelKey: "nav.rechargesJeu", icon: Zap, href: "/marche?type=recharge_jeu" },
  { labelKey: "nav.comptesAbonnements", icon: Tv, href: "/marche?type=abonnement_service" },
  { labelKey: "nav.accessoires", icon: Gamepad2, href: "/marche?type=accessoire" },
  { labelKey: "nav.vetementsGuildes", icon: Shirt, href: "/marche?type=vetement" },
...
const QUICK_ACCESS = [
  { labelKey: "nav.marche", icon: Store, href: "/marche" },
  { labelKey: "nav.accessoiresCourt", icon: Gamepad2, href: "/marche?type=accessoire" },
  { labelKey: "nav.vetementsGuildes", icon: Shirt, href: "/marche?type=vetement" },
  { labelKey: "nav.abonnementsCourt", icon: Tv, href: "/marche?type=abonnement_service" },
  { labelKey: "nav.iaAssistantCourt", icon: Sparkles, href: "/ia" },
  { labelKey: "nav.tournois", icon: Trophy, href: "/tournois" },
  { labelKey: "nav.formation", icon: GraduationCap, href: "/formation/createurs" },
  { labelKey: "nav.marketingDigital", icon: Tv, href: "/marketing-digital" },
];
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            <button onClick={() => setLangueMenuOuvert((o) => !o)} aria-label={t("nav.langue")}>
            ...
            {langueMenuOuvert && (
              <>
                <button
                  ...
                  onClick={() => setLangueMenuOuvert(false)}
                  aria-label={t("common.fermer")}
                  style={{ background: "transparent" }}
                />
                <div
                  className="absolute right-0 top-8 z-50 rounded-xl overflow-hidden shadow-lg"
                  style={{ background: COLORS.background, border: `1px solid ${COLORS.border}`, minWidth: 150 }}
                >
                  {LANGUES_AFFICHAGE.map(({ code, nom }) => (
                    <button
                      key={code}
                      onClick={() => { setLangue(code); setLangueMenuOuvert(false); }}
                      className="w-full text-left px-3 py-2.5 text-sm"
                      style={{
                        background: langue === code ? COLORS.accentPrimary : "transparent",
                        color: langue === code ? COLORS.background : COLORS.textPrimary,
                      }}
                    >
                      {nom}
                    </button>
          ...
          >
            ...
            <nav className="flex flex-col gap-1">
              ...
              <div className="mt-4 pt-4" style={{ borderTop: `1px solid ${COLORS.border}` }}>
                <p className="text-sm mb-2" style={{ color: COLORS.textMuted }}>{t("nav.langue")}</p>
                <div className="grid grid-cols-3 gap-1.5">
                  {LANGUES_AFFICHAGE.map(({ code, nom }) => (
                    <button
                      ...
                      style={{
                        background: langue === code ? COLORS.accentPrimary : COLORS.surface,
      ...
      <main className="flex-1 pt-16 pb-20 px-4 max-w-md mx-auto w-full">
        ...
        <section className="grid grid-cols-4 gap-3 mt-5">
          {QUICK_ACCESS.map(({ labelKey, icon: Icon, badge, href }) => (
            <Link
              href={href}
              key={labelKey}
        ...
        <section className="mt-6">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-lg font-bold" style={{ color: COLORS.accentPrimary }}>
              {t("home.produitsTendances")}
            </h2>
            <ChevronRight size={18} color={COLORS.textMuted} />
          </div>
          <div className="flex gap-3 overflow-x-auto pb-1">
            {produitsTendances.length === 0 && (
              <p className="text-xs" style={{ color: COLORS.textMuted }}>{t("home.aucunProduit")}</p>
            )}
        ...
        >
          <VitrineRotative theme={theme} index={indexVitrine} />
          <div className="relative z-10">
            <h3 className="text-lg font-bold">{t("home.rejoignezEcosysteme")}</h3>
            <p className="text-sm mt-1" style={{ color: COLORS.textMuted }}>
              {t("home.devenezVendeurAgree")}
            </p>
            <Link
              href="/vendre"
              className="mt-3 inline-block px-4 py-2 rounded-lg text-sm font-semibold takca-cta-pulse"
              style={{ background: COLORS.accentPrimary, color: COLORS.background }}
            >
              {t("home.creerCompteVendeur")}
            </Link>
          </div>
        </section>
      </main>
