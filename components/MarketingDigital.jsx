...
// Valeurs stables (indépendantes de la langue affichée), voir
// libelleType() dans le composant pour la traduction à l'affichage.
const TYPES = ["tous", "ebooks", "templates", "formations_video", "livres_physiques"];
const CLE_TYPE = { tous: "common.tous", ebooks: "mktg.typeEbooks", templates: "mktg.typeTemplates", formations_video: "mktg.typeFormationsVideo", livres_physiques: "mktg.typeLivresPhysiques" };
    ...
    <div style={{ background: COLORS.background, minHeight: "100vh", color: COLORS.textPrimary }}>
      ...
      >
        ...
        <span className="text-sm font-semibold flex items-center gap-1">
          <BookOpen size={15} color={COLORS.accentPrimary} /> {t("nav.marketingDigital")}
        </span>
      ...
      <div className="fixed top-14 left-0 right-0 z-30 px-4 py-2 flex flex-col gap-2" style={{ background: COLORS.background, borderBottom: `1px solid ${COLORS.border}` }}>
        <div className="flex items-center gap-2 rounded-lg px-3 py-2" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
          ...
          <input
            ...
            placeholder={t("mktg.chercherPlaceholder")}
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {TYPES.map((valeur) => (
            <button
              ...
              style={{
                background: type === valeur ? COLORS.accentPrimary : COLORS.surface,
                color: type === valeur ? COLORS.background : COLORS.textMuted,
      ...
      <main className="max-w-md mx-auto w-full px-4 pt-32 pb-10 flex flex-col gap-3">
        ...
        {filtres.length === 0 && (
          <p className="text-sm text-center py-16" style={{ color: COLORS.textMuted }}>{t("mktg.aucunResultatPoint")}</p>
        )}
      </main>
    </div>
  );
