...
import {
  ...
  Sun, Moon, ChevronDown, Truck, Flag
} from "lucide-react";
...
export default function BoutiqueVendeur() {
  ...
  useEffect(() => {
    ...
    const charger = async () => {
      ...
      const { data: v } = await supabase
        .from("vendeurs")
        .select("id, nom_boutique, sous_domaine, niveau, note_moyenne, nb_avis, nb_ventes, temps_reponse_moyen_minutes, date_creation, derniere_activite, en_pause, message_pause")
      ...
      setVendeur({
        ...
        tempsReponse: v.temps_reponse_moyen_minutes ? t("boutique.repondEnMoins", { min: v.temps_reponse_moyen_minutes }) : t("boutique.tempsReponseInconnu"),
        depuis: t("boutique.surPlateformeDepuis", { date: new Date(v.date_creation).toLocaleDateString("fr-FR", { month: "long", year: "numeric" }) }),
        enLigne: v.derniere_activite ? (Date.now() - new Date(v.derniere_activite).getTime()) < 5 * 60 * 1000 : false,
        enPause: v.en_pause || false,
        messagePause: v.message_pause || null,
      });
      ...
      if (prod) {
        setProduits(
          prod.map((p) => ({
            ...
            categorie: p.categories_taxes?.nom_categorie || "autre",  // clé technique stable, traduite via libelleCategorie()
          }))
        );
      }
      ...
      const { data: av } = await supabase
        ...
        .limit(10);
      if (av) {
        setAvis(av.map((a) => ({ client: a.users?.nom || t("discussion.client"), note: a.note, commentaire: a.commentaire })));
      }
  ...
  const demarrerConversation = async () => {
    ...
    try {
      ...
      if (!reponse.ok) throw new Error(data.error || t("profilVendeur.impossibleOuvrirConv"));
      router.push(`/messages/${data.conversationId}`);
    } catch (e) {
      setOuvertureConversation(false);
      setErreurConversation(e.message || t("profilVendeur.impossibleOuvrirConv"));
    }
  };
  ...
  const libelleCategorie = (cle) => {
    if (cle === "tous") return t("common.tous");
    const map = { recharge_jeu: "produits.recharge", accessoire: "produits.accessoire", vetement: "produits.vetement", abonnement_service: "produits.abonnement", ebook: "produits.ebook", template: "produits.template" };
    return map[cle] ? t(map[cle]) : t("boutique.autreCategorie");
  };
  ...
  if (!vendeur) {
    return (
      <div style={{ background: COLORS.background, minHeight: "100vh", color: COLORS.textPrimary }} className="flex items-center justify-center px-6 text-center">
        <p className="text-sm" style={{ color: COLORS.textMuted }}>{t("boutique.boutiqueIntrouvable")}</p>
      </div>
    );
  }
      ...
      <main className="max-w-md mx-auto w-full px-4 pt-16 pb-10">
        ...
        >
          <div className="flex items-center gap-3">
            <div className="w-16 h-16 rounded-xl flex-shrink-0 relative" style={{ background: COLORS.background, border: `1px solid ${COLORS.border}` }}>
              <span
                className="absolute -bottom-0.5 -right-0.5 w-4 h-4 rounded-full"
                style={{ background: vendeur.enLigne ? "#3A8A5C" : COLORS.textMuted, border: `2px solid ${COLORS.surface}` }}
                aria-label={vendeur.enLigne ? t("profilVendeur.enLigne") : t("profilVendeur.horsLigne")}
                title={vendeur.enLigne ? t("profilVendeur.enLigne") : t("profilVendeur.horsLigne")}
              />
            </div>
            <div className="flex-1">
              ...
              <p className="text-[11px] mt-0.5" style={{ color: vendeur.enLigne ? "#3A8A5C" : COLORS.textMuted }}>
                {vendeur.enLigne ? t("profilVendeur.enLigne") : t("profilVendeur.horsLigne")}
              </p>
              <div className="flex items-center gap-1 mt-0.5">
                <Star size={13} color={COLORS.accentSecondary} fill={COLORS.accentSecondary} />
                <span className="text-xs" style={{ color: COLORS.textMuted }}>
                  {vendeur.note} {t("boutique.avisSuffix", { count: vendeur.nbAvis })}
                </span>
              </div>
          ...
          {vendeur.enPause && (
            <div className="rounded-lg px-3 py-2 mt-3 text-[11px]" style={{ background: "rgba(178,58,46,0.1)", color: "#B23A2E" }}>
              ⏸ {vendeur.messagePause || t("boutique.boutiqueFermeeTemp")}
            </div>
          )}
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            ...
            <span
              ...
              style={{ background: COLORS.background, color: COLORS.accentSecondary, border: `1px solid ${COLORS.border}` }}
            >
              <ShoppingBag size={12} /> {t("boutique.ventesSuffix", { count: vendeur.nbVentes })}
            </span>
            {vendeur.niveau === "revendeur_officiel" && (
              <span
          ...
          <button
            ...
            style={{ background: COLORS.accentPrimary, color: COLORS.background }}
          >
            <MessageCircle size={16} /> {ouvertureConversation ? t("dashboard.ouvertureEnCours") : t("boutique.discuterAvecVendeur")}
          </button>
          {erreurConversation && (
            <p className="text-[11px] mt-2 text-center" style={{ color: "#B23A2E" }}>{erreurConversation}</p>
          )}
          <Link
            href={`/contact?vendeur=${vendeur.id}`}
            className="w-full mt-2 rounded-xl py-2 text-xs flex items-center justify-center gap-1.5"
            style={{ color: COLORS.textMuted }}
          >
            <Flag size={12} /> {t("profilVendeur.signalerVendeur")}
          </Link>
        </section>
        ...
        <section className="grid grid-cols-2 gap-3 mb-6">
          {produitsFiltres.length === 0 && (
            <p className="text-xs col-span-2" style={{ color: COLORS.textMuted }}>{t("boutique.aucunProduitCategorie")}</p>
          )}
        ...
        <section className="mb-6">
          <h2 className="text-base font-bold mb-3" style={{ color: COLORS.accentPrimary }}>{t("boutique.avisClients")}</h2>
          <div className="flex flex-col gap-2">
            {avis.length === 0 && (
              <p className="text-xs" style={{ color: COLORS.textMuted }}>{t("boutique.aucunAvis")}</p>
            )}
            {avis.map((a, i) => (
        ...
        >
          ...
          <div>
            <p className="text-xs font-semibold" style={{ color: COLORS.textPrimary }}>{t("compte.politiqueLivraisonRetours")}</p>
            <p className="text-[11px] mt-1" style={{ color: COLORS.textMuted }}>
              {t("boutique.politiqueLivraisonDesc")}
            </p>
          </div>
        </section>
      </main>
