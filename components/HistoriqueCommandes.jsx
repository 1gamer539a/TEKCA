...
function StatutBadge({ COLORS, statut, t }) {
  const map = {
    ...
    paye: { label: t("commandes.statutPaye"), color: COLORS.accentSecondary },
    expedie: { label: t("commandes.statutExpedie"), color: COLORS.accentPrimary },
    livre: { label: t("commandes.statutLivre"), color: "#3A8A5C" },
    annule: { label: t("commandes.statutAnnule"), color: "#B23A2E" },
  };
  const s = map[statut];
  return (
...
export default function HistoriqueCommandes() {
  ...
  // "filtre" garde des valeurs stables (pas les libellés traduits) pour
  // que la comparaison ne casse pas selon la langue affichée.
  ...
  const filtres = [
    ...
    { valeur: "encours", cle: "commandes.filtreEnCours" },
    { valeur: "livrees", cle: "commandes.filtreLivrees" },
    { valeur: "annulees", cle: "commandes.filtreAnnulees" },
  ];
  const enCoursStatuts = ["en_attente", "paye", "expedie"];

  const annuler = async (id) => {
    setAnnulationEnCours(id);
    const { data: { session } } = await supabase.auth.getSession();
    const reponse = await fetch(`/api/commandes/${id}/annuler`, {
      method: "POST",
      headers: { Authorization: `Bearer ${session?.access_token}` },
    });
    if (reponse.ok) {
      setCommandes((prev) => prev.map((c) => (c.id === id ? { ...c, statut: "annule" } : c)));
    }
  ...
  const commandesFiltrees = commandes.filter((c) => {
    ...
    if (filtre === "encours") return enCoursStatuts.includes(c.statut);
    if (filtre === "livrees") return c.statut === "livre";
    if (filtre === "annulees") return c.statut === "annule";
    return true;
  });
  ...
  return (
    <div style={{ background: COLORS.background, minHeight: "100vh", color: COLORS.textPrimary }}>
      ...
      >
        ...
        <span className="text-sm font-semibold">{t("compte.mesCommandes")}</span>
      ...
      <main className="max-w-md mx-auto w-full px-4 pt-28 pb-10 flex flex-col gap-3">
        {chargement && (
          <p className="text-sm text-center py-16" style={{ color: COLORS.textMuted }}>{t("common.chargement")}</p>
        )}
        {!chargement && commandesFiltrees.length === 0 && (
          <div className="flex flex-col items-center pt-16 gap-2">
            ...
            <p className="text-sm" style={{ color: COLORS.textMuted }}>{t("commandes.aucuneCommande")}</p>
          </div>
        )}
