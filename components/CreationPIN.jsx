...
export default function CreationPIN({ mode: modeProp }) {
  ...
  const appelApi = async (endpoint, body) => {
    ...
    if (!session?.access_token) {
      // CORRECTIF — juste après une redirection serveur du middleware
      // (chargement de page complet, pas une navigation interne), le
      // SDK Supabase peut ne pas avoir fini de réhydrater la session
      // en mémoire au moment où on tape "Valider" : le premier appel
      // partait alors sans jeton, échouait sans message clair, et
      // seul un rafraîchissement de page (qui relit la session depuis
      // zéro) débloquait la situation. On redonne une seconde chance
      // à la lecture de session avant d'abandonner.
      await new Promise((r) => setTimeout(r, 400));
      ({ data: { session } } = await supabase.auth.getSession());
    }
    if (!session?.access_token) {
      return { ok: false, data: { error: "Session expirée, recharge la page et réessaie." } };
    }
