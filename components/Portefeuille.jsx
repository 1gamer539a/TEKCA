...
import { operateursPourPays } from "../lib/operateurs";
...
export default function Portefeuille() {
  ...
  // CORRECTIF — remplace la liste figée à 2 opérateurs (MTN/Airtel
  // uniquement) par la vraie liste SebPay, adaptée au pays choisi
  // (voir lib/operateurs.js). Se resynchronise à chaque changement de
  // pays via l'effet plus bas, pour ne jamais rester bloqué sur un
  // opérateur qui n'existe pas dans le pays sélectionné.
  const operateursDisponibles = operateursPourPays(paysWallet);
  ...
  // Change de pays → réaligne l'opérateur sélectionné sur la liste
  // valable pour ce pays (évite de rester coincé sur un opérateur
  // qui n'existe pas là où on vient de basculer).
  useEffect(() => {
    const dispo = operateursPourPays(paysWallet);
    if (!dispo.some((o) => o.operateur === moyen)) {
      setMoyen(dispo[0]?.operateur || "mtn");
    }
  }, [paysWallet]);
  ...
  const verifierDestinataire = async () => {
    ...
    try {
      ...
      if (!ok) throw new Error(data.error || t("wallet.destinataireIntrouvable"));
  ...
  const soumettre = async () => {
    ...
    try {
      if (panneau === "recharge") {
        if (parseFloat(montant) < SEUIL_DEPOT_MIN) {
          setErreur(t("wallet.montantMinRecharge", { seuil: SEUIL_DEPOT_MIN.toLocaleString() }));
          setEnCours(false);
          return;
        }
        // On NE retire plus le "0" initial du numéro local avant
        // d'ajouter l'indicatif — un test réel a montré que SebPay
        // rejette le numéro sans ce 0 ("Invalid phone number
        // 24267803987"). À reconfirmer avec un nouveau test : si ça
        // repasse en erreur "Invalid phone number", il faudra revoir
        // encore le format exact attendu par SebPay pour le Congo.
        const numeroComplet = `${indicatifPourPays(paysWallet)}${numero.replace(/[\s-]/g, "")}`;
        // "moyen" contient déjà le slug attendu par SebPay (ex: "mtn",
        // "orange", "wave" — voir lib/operateurs.js, table officielle
        // confirmée). On l'envoie tel quel, PAS le libellé affiché
        // ("MTN Money") — un ancien correctif faisait l'inverse par
        // erreur, en se basant sur un commentaire de doc obsolète.
        const { ok, data } = await appelApi("/api/wallet/recharger", { montant, numeroClient: numeroComplet, operateur: moyen, pays: paysWallet });
        if (!ok) throw new Error(data.error || t("wallet.echecRecharge"));
        setSucces(data.lienPaiement ? t("wallet.redirectionPaiement") : t("wallet.rechargeInitiee"));
        if (data.lienPaiement) window.location.href = data.lienPaiement;
      }

      if (panneau === "retrait") {
        if (parseFloat(montant) < SEUIL_RETRAIT_MIN) {
          setErreur(t("wallet.montantMinRetrait", { seuil: SEUIL_RETRAIT_MIN.toLocaleString() }));
          setEnCours(false);
          return;
        }
        if (parseFloat(montant) >= SEUIL_CNI && !pieceVerifiee) {
          setErreur(t("wallet.verificationRequiseSeuil", { seuil: SEUIL_CNI.toLocaleString() }));
          setEnCours(false);
          return;
        }
        // Idem recharge : on garde le "0" initial (voir commentaire ci-dessus).
        const numeroComplet = `${indicatifPourPays(paysWallet)}${numero.replace(/[\s-]/g, "")}`;
        // Idem recharge : "moyen" est déjà le bon slug, on ne le
        // remplace plus par son libellé avant l'envoi à l'API.
        const { ok, data } = await appelApi("/api/wallet/retirer", { montant, moyen, numeroDestinataire: numeroComplet, operateur: moyen, pays: paysWallet });
        if (!ok) {
          ...
          throw new Error(data.message || data.error || t("wallet.echecRetrait"));
        }
        setSucces(t("wallet.retraitEnvoye"));
        charger();
      }
      ...
      if (panneau === "transfert") {
        ...
        if (!ok) throw new Error(data.error || t("wallet.echecTransfert"));
        setSucces(t("wallet.transfertEnvoyeA", { destinataire: data.destinataire }));
        setDestinataireVerifie(null);
        charger();
      }
  ...
  return (
    <div style={{ background: COLORS.background, minHeight: "100vh", color: COLORS.textPrimary }}>
        ...
        <div className="rounded-2xl p-5 text-center" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, "--sk1": theme === "sombre" ? "#1a2740" : "#eef1f5", "--sk2": theme === "sombre" ? "#26375a" : "#dde3ea" }}>
          ...
          {!chargement && !pieceVerifiee && (
            <p className="text-[11px] mt-2 flex items-center justify-center gap-1" style={{ color: COLORS.textMuted }}>
              <ShieldAlert size={12} /> {t("wallet.identiteNonVerifiee", { seuil: SEUIL_CNI.toLocaleString() })}
            </p>
          )}
        </div>
            ...
            {panneau === "recharge" && (
              <>
                ...
                <div className="flex gap-2 flex-wrap">
                  {operateursDisponibles.map(({ operateur, label }) => (
                    <button
                      key={operateur}
                      onClick={() => setMoyen(operateur)}
                      className="flex-1 rounded-lg py-2 text-[10px] font-semibold uppercase"
                      style={{ background: moyen === operateur ? COLORS.accentPrimary : COLORS.background, color: moyen === operateur ? COLORS.background : COLORS.textMuted, border: `1px solid ${COLORS.border}` }}
                    >
                      {label}
                    </button>
                  ))}
              ...
              <>
                ...
                {parseFloat(montant) >= SEUIL_CNI && !pieceVerifiee && (
                  <button
                    ...
                    style={{ color: "#B23A2E" }}
                  >
                    <ShieldAlert size={12} /> {t("wallet.verificationRequiseAppuie")}
                  </button>
                )}
              </>
            )}
            ...
            {panneau === "transfert" && (
              <>
                <input
                  ...
                  onChange={(e) => { setIdentifiantSaisi(e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, "")); setDestinataireVerifie(null); }}
      ...
      <ConfirmationPIN
        ...
        onSuccess={() => { setPinEnAttente(false); soumettre(); }}
      />
    </div>
  );
}
