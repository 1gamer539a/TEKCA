...
export default function Portefeuille() {
  ...
  useEffect(() => {
    ...
    if (!dispo.some((o) => o.operateur === moyen)) {
      setMoyen(dispo[0]?.operateur || "mtn_mobile_money");
    }
  }, [paysWallet]);
  ...
  const soumettre = async () => {
    ...
    try {
      if (panneau === "recharge") {
        ...
        // Le libellé complet (ex: "MTN Mobile Money") est envoyé à
        // SebPay tel quel — même format qu'avant ce correctif, voir
        // la note de vérification dans lib/operateurs.js.
        const libelleOperateur = operateursDisponibles.find((o) => o.operateur === moyen)?.label || moyen;
        const { ok, data } = await appelApi("/api/wallet/recharger", { montant, numeroClient: numeroComplet, operateur: libelleOperateur, pays: paysWallet });
      ...
      if (panneau === "retrait") {
        ...
        const numeroComplet = `${indicatifPourPays(paysWallet)}${numero.replace(/[\s-]/g, "").replace(/^0+/, "")}`;
        const libelleOperateurRetrait = operateursDisponibles.find((o) => o.operateur === moyen)?.label || moyen;
        const { ok, data } = await appelApi("/api/wallet/retirer", { montant, moyen, numeroDestinataire: numeroComplet, operateur: libelleOperateurRetrait, pays: paysWallet });
        if (!ok) {
