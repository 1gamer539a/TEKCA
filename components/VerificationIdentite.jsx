...
export default function VerificationIdentite() {
  ...
  const envoyer = async () => {
    ...
    try {
      ...
      const { data: signalement, error: erreurInsertion } = await supabase.from("contacts_support").insert({
        client_id: user.id,
        motif: "verification_identite",
        message: `Demande de vérification d'identité — pièce jointe : ${urlPreuve.publicUrl}`,
        statut: "ouvert",
      }).select().single();
      if (erreurInsertion) throw erreurInsertion;

      setSignalementId(signalement.id);
      setDejaEnvoye(true);
    } catch (e) {
      setErreur(e.message || t("vendeur.erreurEnvoi"));
    } finally {
  ...
  if (dejaVerifie) {
    return (
      <div style={{ background: COLORS.background, minHeight: "100vh", color: COLORS.textPrimary }} className="flex flex-col items-center justify-center px-6 text-center gap-2">
        ...
        <p className="font-bold">{t("verifIdentite.identiteDejaVerifieeTitre")}</p>
        <p className="text-sm" style={{ color: COLORS.textMuted }}>{t("verifIdentite.retraitsSansLimite")}</p>
      </div>
    );
  }
  ...
  if (dejaEnvoye) {
    return (
      <div style={{ background: COLORS.background, minHeight: "100vh", color: COLORS.textPrimary }} className="flex flex-col items-center justify-center px-6 text-center gap-2">
        ...
        <p className="font-bold">{t("verifIdentite.documentEnvoye")}</p>
        <p className="text-sm" style={{ color: COLORS.textMuted }}>{t("verifIdentite.equipeVerifieDesc")}</p>
        {signalementId && (
          <button
  ...
  return (
    <div style={{ background: COLORS.background, minHeight: "100vh", color: COLORS.textPrimary }}>
      ...
      >
        ...
        <span className="text-sm font-semibold">{t("verifIdentite.titre")}</span>
      ...
      <main className="max-w-md mx-auto w-full px-4 pt-20 pb-10 flex flex-col gap-4">
        <div className="rounded-2xl p-4" style={{ background: COLORS.surface, border: `1px solid ${COLORS.accentPrimary}` }}>
          <div className="flex items-center gap-2 mb-1">
            ...
            <p className="text-sm font-bold">{t("verifIdentite.pourquoiDemande")}</p>
          </div>
          <p className="text-xs" style={{ color: COLORS.textMuted }}>
            {t("verifIdentite.retraitsNecessitentDesc")}
          </p>
        </div>

        <div className="rounded-2xl p-4" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
          <label className="text-xs font-semibold block mb-2">{t("verifIdentite.pieceIdentiteCni")}</label>
          {fichier && fichier.type.startsWith("image/") && (
            <img
              src={URL.createObjectURL(fichier)}
              alt={t("verifIdentite.apercuPiece")}
              className="w-full max-h-48 rounded-lg object-cover mb-2"
          ...
          >
            ...
            {fichier ? `✓ ${fichier.name}` : t("verifIdentite.importerPhotoScan")}
          </label>
        </div>
        ...
        >
          {envoiEnCours ? t("contact.envoiEnCoursMot") : t("verifIdentite.envoyerPourVerification")}
        </button>
      </main>
    </div>
  );
