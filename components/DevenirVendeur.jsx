...
export default function DevenirVendeur() {
  ...
  // CORRECTIF — Le Marché était figé sur pays: "CG" en dur à l'envoi
  // (voir soumettreMarche plus bas), alors que le code de livraison à
  // 6 caractères remis en main propre ne dépend d'aucune logistique
  // liée au Congo précisément : il fonctionne pareil dans n'importe
  // quel pays. Ouvre donc aux mêmes 19 pays que le reste de la
  // plateforme.
  const [paysMarche, setPaysMarche] = useState("CG");
...
const PAYS_DISPONIBLES = PAYS_SEBPAY;
  ...
  const soumettreMarche = async () => {
    setErreur(null);
    // CORRECTIF — même problème que pour "Envoyer ma demande" : un
    // champ manquant faisait juste "return" sans rien dire, le
    // bouton semblait mort. On dit maintenant précisément ce qui
    // manque, comme pour soumettreRevendeurOfficiel juste en dessous.
    const manquants = [];
    if (!nomMarche) manquants.push("ton nom");
    if (!telMarche) manquants.push("ton téléphone");
    if (!villeMarche) manquants.push("ta ville");
  ...
  const soumettreRevendeurOfficiel = async () => {
    setErreur(null);
    // CORRECTIF — avant, un champ obligatoire manquant faisait juste
    // "return" sans rien afficher : le bouton semblait mort. On dit
    // maintenant précisément ce qui manque.
    const manquants = [];
    if (!nomBoutique) manquants.push("le nom de la boutique");
    if (!sousDomaine) manquants.push("le sous-domaine");
    if (categories.length === 0) manquants.push("au moins une catégorie");
    if (!fichierIdentite) manquants.push("la pièce d'identité");
    if (!fichierActivite) manquants.push("le justificatif d'activité");
    if (manquants.length > 0) {
      setErreur(`Il manque : ${manquants.join(", ")}.`);
      return;
    }
        ...
        {niveau === "vendeur_simple" && (
          <div className="flex flex-col gap-4">
            ...
            <div className="rounded-2xl p-4 flex flex-col gap-3" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
              ...
              <div>
                <label className="text-xs font-semibold flex items-center gap-1 mb-1"><MapPin size={12} /> Pays</label>
                <select
                  value={paysMarche}
                  onChange={(e) => setPaysMarche(e.target.value)}
                  className="w-full rounded-lg px-3 py-2 text-sm outline-none"
                  style={{ background: COLORS.background, color: COLORS.textPrimary, border: `1px solid ${COLORS.border}` }}
                >
                  {PAYS_SEBPAY.map((p) => (
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={`${indicatifPourPays(paysMarche)} 06 000 00 00`}
                  className="w-full rounded-lg px-3 py-2 text-sm outline-none"
                  style={{ background: COLORS.background, color: COLORS.textPrimary, border: `1px solid ${COLORS.border}` }}
                />
              </div>
            ...
            >
              {envoiEnCours ? "Envoi..." : "Commencer à vendre"}
            </button>
