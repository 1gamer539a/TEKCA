...
export default function PageMessages() {
      ...
      if (!error && data) {
        const conv = data.map((c) => {
          ...
          return {
            id: c.id,
            type: "vendeur",
            nom: c.vendeurs?.nom_boutique || t("messages.vendeurDefaut"),
            dernier: dernier?.contenu || t("messages.nouvelleConversation"),
  ...
  const CONVERSATION_IA = { id: "ia", type: "ia", nom: t("messages.iaAssistantNom"), dernier: t("messages.poseUneQuestion"), temps: "", nonLu: 0 };
  ...
  if (connecte === false) {
    return (
      <div style={{ background: COLORS.background, minHeight: "100vh", color: COLORS.textPrimary }} className="flex flex-col items-center justify-center px-6 text-center">
        ...
        <p className="font-bold text-lg">{t("messages.connecteToiTitre")}</p>
        <p className="text-sm mt-2" style={{ color: COLORS.textMuted }}>
          {t("messages.connecteToiDesc")}
        </p>
        <Link
          href="/auth"
        ...
        >
          <LogIn size={16} /> {t("messages.seConnecterCreerCompte")}
        </Link>
      </div>
    );
  }
  ...
  return (
    <div style={{ background: COLORS.background, minHeight: "100vh", color: COLORS.textPrimary }}>
      ...
      >
        ...
        <span className="text-sm font-semibold">{t("messages.messagesTitre")}</span>
      ...
      <main className="max-w-md mx-auto w-full px-4 pt-20 pb-10">
        <div className="flex items-center gap-2 rounded-lg px-3 py-2 mb-4" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
          ...
          <input
            ...
            placeholder={t("messages.rechercherConversation")}
        ...
        <div className="flex flex-col gap-1">
          ...
          {filtrees.length === 0 && !chargement && (
            <p className="text-sm text-center py-10" style={{ color: COLORS.textMuted }}>{t("messages.aucuneConversation")}</p>
          )}
          {chargement && (
