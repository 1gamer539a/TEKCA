...
export default function ModifierPIN() {
  ...
  const soumettre = async (nouveauPinFinal) => {
    ...
    try {
      ...
      if (!reponse.ok) {
        setErreur(true);
        setMessageErreur(data.error || t("pin.impossibleChanger"));
      ...
      setEnCours(false);
      setErreur(true);
      setMessageErreur(t("pin.erreurReseauReessaie"));
    }
  };
  ...
  const titres = {
    ancien: t("pin.entreCodeActuel"),
    nouveau: t("pin.choisisNouveauCode"),
    confirmation: t("pin.confirmeNouveauCode"),
    succes: t("pin.codeModifie"),
  };
  ...
  return (
    <div style={{ background: COLORS.background, minHeight: "100vh", color: COLORS.textPrimary }} className="flex flex-col">
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-6 pt-16">
        ...
        {etape === "succes" && (
          <p className="text-xs text-center" style={{ color: COLORS.textMuted }}>{t("pin.codeMisAJourDesc")}</p>
        )}
      </main>
    </div>
  );
