  ...
  useEffect(() => {
    ...
    const charger = async () => {
      ...
      try {
        ...
        if (!reponse.ok) throw new Error(data.error || t("achats.erreurChargement"));
        setAchats(data.achats || []);
      } catch (e) {
        setErreur(e.message || t("achats.impossibleChargerAchats"));
      } finally {
        setChargement(false);
      }
    };
  ...
  return (
    <div className="min-h-screen pb-24" style={{ background: COLORS.background, color: COLORS.textPrimary }}>
      <header className="sticky top-0 z-30 flex items-center justify-between px-4 py-3" style={{ background: COLORS.background, borderBottom: `1px solid ${COLORS.border}` }}>
        ...
        <p className="font-bold text-sm">{t("compte.mesAchatsNumeriques")}</p>
      ...
      {!pinVerifie ? (
        <div className="flex flex-col items-center text-center gap-3 px-6 py-16">
          ...
          <p className="text-sm font-semibold">{t("achats.codesProteges")}</p>
          <p className="text-xs max-w-xs" style={{ color: COLORS.textMuted }}>
            {t("achats.saisisPinDesc")}
          </p>
        </div>
      ) : (
        <main className="px-4 py-4 flex flex-col gap-3">
          {chargement && <p className="text-sm text-center py-10" style={{ color: COLORS.textMuted }}>{t("common.chargement")}</p>}
          {erreur && (
          ...
          {!chargement && !erreur && achats.length === 0 && (
            <div className="flex flex-col items-center text-center gap-2 py-16">
              ...
              <p className="text-sm font-semibold">{t("achats.aucunAchat")}</p>
              <p className="text-xs" style={{ color: COLORS.textMuted }}>
                {t("achats.codesApparaitrontDesc")}
              </p>
            </div>
          )}
          {achats.map((a) => (
            <div key={a.commandeId} className="rounded-xl p-4" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
              ...
              <p className="text-[11px] mt-0.5" style={{ color: COLORS.textMuted }}>
                {t("achats.acheteLe", { date: new Date(a.dateAchat).toLocaleDateString("fr-FR", { day: "numeric", month: "short" }), boutique: a.boutique })}
              </p>
              <button
      ...
      <ConfirmationPIN
        ouvert={!pinVerifie}
        theme={theme}
        titre={t("achats.entrePinPourVoir")}
