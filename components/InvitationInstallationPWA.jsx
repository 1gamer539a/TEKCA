    ...
    <div className="fixed inset-0 z-[95] flex items-end sm:items-center justify-center" style={{ background: "rgba(0,0,0,0.5)" }}>
      <div className="w-full sm:max-w-sm rounded-t-2xl sm:rounded-2xl p-6" style={{ background: COLORS.background, color: COLORS.textPrimary }}>
        <div className="flex justify-between items-start mb-2">
          ...
          <button onClick={() => setVisible(false)} aria-label={t("pwa.fermer")}>
            <X size={20} color={COLORS.textMuted} />
          </button>
        </div>

        <p className="text-base font-bold mb-1">{t("pwa.installeSurTelephone")}</p>
        <p className="text-xs mb-4" style={{ color: COLORS.textMuted }}>
          {t("pwa.accesRapideDesc")}
        </p>
        ...
        {surIOS ? (
          <div className="rounded-xl p-3 flex flex-col gap-2 mb-1" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
            <div className="flex items-center gap-2 text-xs">
              ...
              1. {t("pwa.appuieSurPartager")} <strong>{t("pwa.partagerMot")}</strong> {t("pwa.enBasDeSafari")}
            </div>
            <div className="text-xs pl-[23px]">2. {t("pwa.choisisMot")} <strong>{t("pwa.surEcranAccueil")}</strong></div>
          </div>
        ) : eventInstall ? (
          <button
            onClick={installer}
