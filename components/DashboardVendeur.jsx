...
function StatutBadge({ COLORS, statut, t }) {
  const map = {
    valide: { label: t("dashboard.statutValide"), color: COLORS.accentSecondary },
    en_attente: { label: t("commandes.statutEnAttente"), color: COLORS.textMuted },
    refuse: { label: t("dashboard.statutRefuse"), color: "#B23A2E" },
    expedie: { label: t("commandes.statutExpedie"), color: COLORS.accentPrimary },
  };
...
export default function DashboardVendeur() {
  ...
  const ajouterCodeUnitaire = async () => {
    ...
    if (error) {
      setMessageCoffreFort({ type: "erreur", texte: error.code === "23505" ? t("dashboard.codeExisteDeja") : t("dashboard.erreurAjout") });
      return;
    }
    setCodeManuel("");
    setMessageCoffreFort({ type: "succes", texte: t("dashboard.codeAjoute") });
    chargerCoffreFort();
  };
  ...
  const importerCodesEnMasse = async () => {
    ...
    if (error) {
      setMessageCoffreFort({ type: "erreur", texte: t("dashboard.erreurImport") });
      return;
    }
    setMessageCoffreFort({ type: "succes", texte: t("dashboard.codesImportes", { count: count ?? uniques.length }) });
    setTexteImport("");
  ...
  const validerCode = async (commandeId) => {
    ...
    try {
      ...
      setMessagesValidation((prev) => ({ ...prev, [commandeId]: { type: "succes", texte: t("dashboard.livraisonConfirmee") } }));
      setCommandes((prev) => prev.map((c) => (c.id === commandeId ? { ...c, statut: "livre" } : c)));
    } catch (e) {
      setMessagesValidation((prev) => ({ ...prev, [commandeId]: { type: "erreur", texte: t("dashboard.erreurReseauCourt") } }));
    } finally {
  ...
  useEffect(() => {
    ...
    const chargerConversations = async () => {
      ...
      const listes = (data || []).map((c) => {
        ...
        return {
          id: c.id,
          client: c.users?.nom || t("discussion.client"),
          dernierMessage: dernier?.contenu || t("messages.nouvelleConversation"),
          nonLus,
        };
      });
      ...
      try {
        ...
        if (commandesData) {
          setCommandes(commandesData.map((c) => ({
            id: c.id,
            client: c.client_id,
            clientNom: c.users?.nom || t("discussion.client"),
  ...
  const TABS = [
    { id: "apercu", label: t("dashboard.tabApercu"), icon: LayoutDashboard },
    { id: "produits", label: t("dashboard.tabProduits"), icon: Package },
    { id: "coffre_fort", label: t("dashboard.tabCoffreFort"), icon: KeyRound },
    { id: "commandes", label: t("dashboard.tabCommandes"), icon: ShoppingBag },
    { id: "messages", label: t("dashboard.tabMessages"), icon: MessageSquare },
    { id: "stats", label: t("dashboard.tabStats"), icon: BarChart3, premiumOnly: true },
    { id: "paiements", label: t("dashboard.tabPaiement"), icon: CreditCard },
    { id: "parametres", label: t("dashboard.tabReglages"), icon: Settings },
  ];
  ...
  if (!chargement && !vendeurId) {
    return (
      <div style={{ background: COLORS.background, minHeight: "100vh", color: COLORS.textPrimary }} className="flex flex-col items-center justify-center px-6 text-center gap-3">
        <p className="font-bold text-lg">{t("dashboard.aucunProfilVendeur")}</p>
        <p className="text-sm" style={{ color: COLORS.textMuted }}>
          {t("dashboard.connecteToiVendeurDesc")}
        </p>
  ...
  return (
    <div style={{ background: COLORS.background, minHeight: "100vh", color: COLORS.textPrimary }}>
      ...
      >
        <div>
          <p className="text-sm font-bold">{statsVendeur.nomBoutique || t("dashboard.maBoutique")}</p>
          <div className="flex items-center gap-1">
            {niveau === "revendeur_officiel" && <ShieldCheck size={12} color={COLORS.accentPrimary} />}
            <span className="text-[10px]" style={{ color: COLORS.textMuted }}>
              {niveau === "revendeur_officiel" ? t("dashboard.revendeurOfficiel") : t("dashboard.vendeurSimple")}
            </span>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <Link href="/notifications" className="relative" aria-label={t("notifications.titre")}>
      ...
      <main className="max-w-md mx-auto w-full px-4 pt-20 pb-24">
          ...
          <div className="flex flex-col gap-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-xl p-3" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
                <p className="text-[11px]" style={{ color: COLORS.textMuted }}>{t("dashboard.ventesTotales")}</p>
                <p className="text-lg font-bold" style={{ color: COLORS.accentPrimary }}>{statsVendeur.nbVentes ?? 0}</p>
              </div>
              <div className="rounded-xl p-3" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
                <p className="text-[11px]" style={{ color: COLORS.textMuted }}>{t("dashboard.commandesEnCours")}</p>
              ...
              <div className="rounded-xl p-3" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
                <div className="flex items-center gap-1">
                  ...
                  <p className="text-[11px]" style={{ color: COLORS.textMuted }}>{t("dashboard.noteMoyenne")}</p>
                </div>
              ...
              <div className="rounded-xl p-3" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
                <div className="flex items-center gap-1">
                  ...
                  <p className="text-[11px]" style={{ color: COLORS.textMuted }}>{t("dashboard.reactivite")}</p>
                </div>
            ...
            <div className="rounded-xl p-3" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
              <p className="text-xs font-semibold mb-1">{t("dashboard.alertes")}</p>
              <p className="text-[11px]" style={{ color: COLORS.textMuted }}>
                {(() => {
                  const nbRefuses = produits.filter((p) => p.statut === "refuse").length;
                  const nbNonLus = conversations.reduce((total, c) => total + (c.nonLus || 0), 0);
                  if (nbRefuses === 0 && nbNonLus === 0) return t("dashboard.rienASignaler");
                  const parties = [];
                  if (nbRefuses > 0) parties.push(t("dashboard.produitsRefuses", { count: nbRefuses }));
                  if (nbNonLus > 0) parties.push(t("dashboard.messagesNonLus", { count: nbNonLus }));
                  return parties.join(" · ");
                })()}
              </p>
            </div>
        ...
        {tab === "produits" && (
          <div className="flex flex-col gap-3" style={{ "--sk1": theme === "sombre" ? "#1a2740" : "#eef1f5", "--sk2": theme === "sombre" ? "#26375a" : "#dde3ea" }}>
            {!chargement && (
              ...
              >
                <Plus size={16} /> {t("dashboard.ajouterUnProduit")}
              </Link>
            )}
            <p className="text-[11px]" style={{ color: COLORS.textMuted }}>
              {t("dashboard.preuveRequiseDesc")}
            </p>
            ...
            {erreurProduits && !chargement && (
              <div className="tekca-shake rounded-xl p-4" style={{ background: theme === "sombre" ? "rgba(178,58,46,0.12)" : "rgba(178,58,46,0.08)", border: "1px solid #B23A2E" }}>
                <p className="text-sm font-semibold" style={{ color: "#B23A2E" }}>{t("dashboard.impossibleChargerProduits")}</p>
                <p className="text-xs mt-0.5" style={{ color: COLORS.textMuted }}>{t("dashboard.verifieConnexion")}</p>
                <button
            ...
            {!chargement && !erreurProduits && produits.length === 0 && (
              <div className="flex flex-col items-center text-center py-10 rounded-xl" style={{ background: COLORS.surface, border: `1px dashed ${COLORS.border}` }}>
                ...
                <p className="text-sm font-semibold mt-3">{t("dashboard.aucunProduitPourInstant")}</p>
                <p className="text-xs mt-1 px-6" style={{ color: COLORS.textMuted }}>{t("dashboard.ajouteTonPremierProduit")}</p>
                <Link
                  href="/produit/nouveau"
                  className="mt-4 px-4 py-2 rounded-lg text-xs font-semibold flex items-center gap-1.5"
                  style={{ background: COLORS.accentPrimary, color: COLORS.background }}
                >
                  <Plus size={14} /> {t("dashboard.ajouterUnProduit")}
                </Link>
              </div>
            )}
              ...
              <div key={p.id} className="rounded-xl p-3" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-[11px]" style={{ color: COLORS.textMuted }}>{t("dashboard.stockLabel", { stock: p.stock })}</p>
                  </div>
                  <StatutBadge COLORS={COLORS} statut={p.statut} t={t} />
                </div>
                <div className="flex items-center gap-3 mt-2">
                  {p.statut === "valide" && (
                    ...
                    >
                      <Rocket size={12} /> {t("dashboard.boosterCetteAnnonce")}
                    </button>
                  )}
                  <button
                    ...
                    style={{ color: "#B23A2E" }}
                  >
                    <Trash2 size={12} /> {t("dashboard.supprimer")}
                  </button>
                </div>
                {produitASupprimer?.id === p.id && (
                  <div className="rounded-lg p-2.5 mt-2" style={{ background: COLORS.background, border: `1px solid ${COLORS.border}` }}>
                    <p className="text-[11px]" style={{ color: COLORS.textMuted }}>{t("dashboard.supprimerDefinitivement", { nom: p.nom })}</p>
                    <div className="flex gap-2 mt-2">
                      ...
                      >
                        {suppressionEnCours ? "..." : t("common.confirmer")}
                      </button>
                    </div>
                  </div>
                )}
        ...
        {tab === "coffre_fort" && (
          <div className="flex flex-col gap-3">
            {chargementCoffreFort && (
              <p className="text-sm text-center py-10" style={{ color: COLORS.textMuted }}>{t("common.chargement")}</p>
            )}
            ...
            {!chargementCoffreFort && vueCoffreFort === "stock" && (
              <>
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-xl p-3" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
                    <p className="text-[11px]" style={{ color: COLORS.textMuted }}>{t("dashboard.codesEnStock")}</p>
                    <p className="text-lg font-bold" style={{ color: COLORS.accentPrimary }}>
                      {produitsDigitaux.reduce((s, p) => s + p.disponible, 0)}
                    </p>
                  </div>
                  <div className="rounded-xl p-3" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
                    <p className="text-[11px]" style={{ color: COLORS.textMuted }}>{t("dashboard.codesVendus")}</p>
                    <p className="text-lg font-bold" style={{ color: COLORS.accentPrimary }}>
                ...
                <p className="text-[11px] font-semibold uppercase mt-1" style={{ color: COLORS.textMuted }}>{t("dashboard.stockParProduit")}</p>
                ...
                {produitsDigitaux.length === 0 && (
                  <div className="rounded-xl p-4 text-center" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
                    <p className="text-xs" style={{ color: COLORS.textMuted }}>
                      {t("dashboard.aucunProduitDigital")}
                    </p>
                  </div>
                )}

                {produitsDigitaux.map((p) => {
                  const label = p.disponible === 0 ? t("produit.epuise") : t("dashboard.dispo", { count: p.disponible });
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-[11px] mt-1" style={{ color: COLORS.textMuted }}>
                        {p.vendu} {t("dashboard.venduMot")}{p.noteMoyenne ? ` · ${p.noteMoyenne}★ (${p.nbAvis} avis)` : ` · ${t("dashboard.pasEncoreAvis")}`}
                      </p>
                    </button>
                  );
                })}
                ...
                <button
                  ...
                  style={{ background: COLORS.accentPrimary, color: COLORS.background }}
                >
                  <Plus size={16} /> {t("dashboard.ajouterDuStock")}
                </button>
              </>
            )}
            ...
            {!chargementCoffreFort && vueCoffreFort === "historique" && produitCoffreFortActif && (
              <>
                <button
                  ...
                  style={{ color: COLORS.textMuted }}
                >
                  <ArrowLeft size={14} /> {t("common.retour")}
                </button>
                <p className="text-sm font-bold">{t("dashboard.historiqueTitre", { nom: produitCoffreFortActif.nom })}</p>
                {historiqueCodes.length === 0 && (
                  <p className="text-xs py-6 text-center" style={{ color: COLORS.textMuted }}>{t("dashboard.aucunCodeEnregistre")}</p>
                )}
                {historiqueCodes.map((c) => (
                  <div key={c.id} className="rounded-xl p-3 flex items-center justify-between" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
                    ...
                    >
                      {c.statut === "vendu" ? t("dashboard.venduLe", { date: new Date(c.date_vente).toLocaleDateString("fr-FR", { day: "numeric", month: "short" }) }) : t("dashboard.disponibleStatut")}
                    </span>
                  </div>
                ))}
                <p className="text-[11px] text-center mt-1" style={{ color: COLORS.textMuted }}>
                  {t("dashboard.chaqueCodeUneFois")}
                </p>
              </>
            )}
            ...
            {vueCoffreFort === "ajouter" && (
              <>
                ...
                <p className="text-sm font-bold mb-1">{t("dashboard.ajouterDuStock")}</p>

                <div>
                  <p className="text-[11px] font-semibold mb-1" style={{ color: COLORS.textMuted }}>{t("dashboard.produitConcerne")}</p>
                  <select
                  ...
                  >
                    <option value="">{t("dashboard.choisirUnProduit")}</option>
                    {produitsDigitaux.map((p) => (
                ...
                <div className="rounded-xl p-4" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
                  <p className="text-xs font-semibold mb-2">{t("dashboard.ajoutManuel")}</p>
                  <input
                    value={codeManuel}
                    onChange={(e) => setCodeManuel(e.target.value)}
                    placeholder={t("dashboard.codeExemplePlaceholder")}
                  ...
                  >
                    {ajoutCodeEnCours ? "..." : t("dashboard.ajouterCeCode")}
                  </button>
                </div>

                <div className="rounded-xl p-4" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
                  <p className="text-xs font-semibold mb-1">{t("dashboard.importEnMasse")}</p>
                  <p className="text-[11px] mb-2" style={{ color: COLORS.textMuted }}>{t("dashboard.fichierCsvTxtDesc")}</p>
                  <label
                  ...
                  >
                    ...
                    <span className="text-[11px]" style={{ color: COLORS.textMuted }}>{t("dashboard.glisserFichier")}</span>
                    <input
                      type="file"
                      accept=".csv,.txt"
                      className="hidden"
                  ...
                  {apercuImport && (
                    <p className="text-[11px] mt-2" style={{ color: COLORS.textMuted }}>
                      {t("dashboard.apercuCodesDetectes", { total: apercuImport.total })}{apercuImport.doublons > 0 ? t("dashboard.doublonsIgnores", { count: apercuImport.doublons }) : "."}
                    </p>
                  )}
                  <button
                  ...
                  >
                    {importEnCours ? t("dashboard.importEnCours") : apercuImport ? t("dashboard.importerNCodes", { count: apercuImport.total }) : t("dashboard.importerBtn")}
                  </button>
                </div>
              </>
            )}
            ...
            {commandes.map((c) => (
              <div key={c.id} className="rounded-xl p-3" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
                <div className="flex items-center justify-between">
                  ...
                  <StatutBadge COLORS={COLORS} statut={c.statut} t={t} />
                </div>
                ...
                >
                  ...
                  {ouvertureConversation === c.id ? t("dashboard.ouvertureEnCours") : t("dashboard.repondreA", { nom: c.clientNom })}
                </button>
                {["paye", "expedie"].includes(c.statut) && (
                  <div className="mt-2 flex items-center gap-2">
                    <input
                      ...
                      placeholder={t("dashboard.codeClientPlaceholder")}
                      maxLength={6}
                    ...
                    <button
                      ...
                      style={{ background: COLORS.accentPrimary, color: COLORS.background }}
                    >
                      {validationEnCours === c.id ? "..." : t("dashboard.valider")}
                    </button>
                  </div>
                )}
                ...
                {niveau === "revendeur_officiel" && (
                  <button className="text-[11px] mt-2 flex items-center gap-1" style={{ color: COLORS.accentPrimary }}>
                    <Truck size={12} /> {t("dashboard.suiviLivraisonPlateforme")}
                  </button>
                )}
                {c.statut === "paye" && (
                  <button
                    ...
                    style={{ color: "#B23A2E" }}
                  >
                    {annulationEnCours === c.id ? t("commandes.annulationEnCours") : t("dashboard.annulerCommande")}
                  </button>
                )}
              </div>
            ))}
        ...
        {tab === "messages" && (
          <div className="flex flex-col gap-3">
            <div className="rounded-xl p-3 flex items-center justify-between" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
              <div>
                <p className="text-xs font-semibold">{t("dashboard.reponseAutoIA")}</p>
                <p className="text-[11px]" style={{ color: COLORS.textMuted }}>{t("dashboard.iaRepondDesc")}</p>
              </div>
              <input type="checkbox" className="w-4 h-4" />
            </div>

            {chargementConversations && (
              <p className="text-xs text-center py-6" style={{ color: COLORS.textMuted }}>{t("common.chargement")}</p>
            )}
            {!chargementConversations && conversations.length === 0 && (
              <p className="text-sm text-center py-10" style={{ color: COLORS.textMuted }}>{t("dashboard.aucunMessage")}</p>
            )}
            {conversations.map((c) => (
              <Link
                key={c.id}
        ...
        {tab === "stats" && (
          niveau === "revendeur_officiel" ? (
            <div className="flex flex-col gap-3">
              <div className="rounded-xl p-4" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
                <p className="text-xs font-semibold mb-2">{t("dashboard.evolutionVentes")}</p>
                <div className="h-24 rounded-lg" style={{ background: COLORS.background }} />
              </div>
              <div className="rounded-xl p-4" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
                <p className="text-xs font-semibold mb-1">{t("dashboard.tauxConversion")}</p>
            ...
            <div className="rounded-xl p-6 flex flex-col items-center text-center gap-2" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
              ...
              <p className="text-sm font-semibold">{t("dashboard.statsAvancees")}</p>
              <p className="text-[11px]" style={{ color: COLORS.textMuted }}>
                {t("dashboard.disponibleRevendeurs")}
              </p>
            </div>
          )
        )}
        ...
        {tab === "paiements" && (
          <div className="flex flex-col gap-3">
            <div className="rounded-xl p-4" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
              <p className="text-xs font-semibold mb-2">{t("dashboard.prestatairePaiement")}</p>
              <select
                className="w-full rounded-lg px-3 py-2 text-sm"
                style={{ background: COLORS.background, color: COLORS.textPrimary, border: `1px solid ${COLORS.border}` }}
              >
                <option>Sebpay</option>
                <option>{t("dashboard.autrePrestataire")}</option>
              </select>
            </div>
            <div className="rounded-xl p-4" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
              <p className="text-xs font-semibold mb-1">{t("dashboard.prelevementsPlateforme")}</p>
              <p className="text-[11px]" style={{ color: COLORS.textMuted }}>
                {niveau === "revendeur_officiel" ? t("dashboard.abonnementCommissionHybride") : t("dashboard.commissionSimple")}
              </p>
            </div>
          </div>
        )}
        ...
        {tab === "parametres" && (
          <div className="flex flex-col gap-3">
            <div className="rounded-xl p-4" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-semibold">{t("dashboard.boutiqueLabel")} {enPause ? t("dashboard.boutiqueEnPause") : t("dashboard.boutiqueOuverte")}</p>
                  <p className="text-[11px] mt-0.5" style={{ color: COLORS.textMuted }}>
                    {t("dashboard.enPauseDesc")}
                  </p>
                </div>
                <button
                  ...
                  aria-label={t("dashboard.basculerPauseAria")}
                >
              ...
              {enPause && (
                <input
                  ...
                  placeholder={t("dashboard.fermeTemporairementPlaceholder")}
            ...
            <div className="rounded-xl p-4" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
              <p className="text-xs font-semibold mb-2">{t("dashboard.sousDomaine")}</p>
            ...
            <div className="rounded-xl p-4" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
              <p className="text-xs font-semibold mb-1">{t("dashboard.politiqueLivraisonRetoursTitre")}</p>
              <p className="text-[11px]" style={{ color: COLORS.textMuted }}>
                {t("dashboard.politiqueDefinieParPlateforme")}
              </p>
            </div>
          </div>
        )}
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" style={{ background: "rgba(0,0,0,0.6)" }}>
          <div className="w-full max-w-md rounded-t-2xl sm:rounded-2xl p-5" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="font-bold text-base">{t("dashboard.boosterTitre", { nom: modalBoost })}</p>
            </div>
            <p className="text-sm mb-4" style={{ color: COLORS.textMuted }}>
              {t("dashboard.boosterDesc")}
            </p>
            <div className="flex flex-col gap-2 mb-4">
              {[
                { duree: "24h", prix: "500 FCFA" },
                { duree: t("dashboard.duree3j"), prix: "1 200 FCFA" },
                { duree: t("dashboard.duree7j"), prix: "2 500 FCFA" },
              ].map((b) => (
                <button
                  key={b.duree}
