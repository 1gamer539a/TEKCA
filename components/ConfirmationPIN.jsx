  ...
  const verifier = async (pinComplet) => {
    ...
    try {
      ...
      if (!session?.access_token) {
        // Même correctif que CreationPIN.jsx : seconde chance à la
        // lecture de session avant d'abandonner.
        await new Promise((r) => setTimeout(r, 400));
        ({ data: { session } } = await supabase.auth.getSession());
      }
      if (!session?.access_token) {
        setErreur(true);
        setMessageErreur("Session expirée, recharge la page et réessaie.");
        setEnCours(false);
        return;
      }
