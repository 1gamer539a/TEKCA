...
*/
export default function ConfirmationPIN({ ouvert, onSuccess, onAnnuler, theme = "clair", titre = "Confirme le paiement" }) {
  ...
  const verifier = async (pinComplet) => {
    ...
    for (let essai = 1; essai <= MAX_ESSAIS; essai++) {
      ...
      if (!session?.access_token) {
        ...
        setErreur(true);
        setMessageErreur("Connexion instable — vérifie ta connexion et réessaie.");
        setEnCours(false);
        return;
      }
    ...
    >
      ...
      >
        ...
        <div className="grid grid-cols-3 gap-3 w-full max-w-[260px]">
          ...
          <button onClick={effacer} disabled={enCours} className="h-14 rounded-full flex items-center justify-center" style={{ color: COLORS.textMuted }} aria-label="Effacer">
            <Delete size={20} />
          </button>
        </div>
