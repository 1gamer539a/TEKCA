...
import { ArrowLeft, Sun, Moon, Lock, User, Mail, Eye, EyeOff, ShieldCheck } from "lucide-react";
...
export default function AuthCompte() {
  ...
  const [paysInscription, setPaysInscription] = useState("CG");
  ...
  const [attenteConfirmationEmail, setAttenteConfirmationEmail] = useState(false);
  ...
  const creerCompte = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.auth.signUp({
        email,
        password: motDePasse,
        options: { data: { nom, telephone: telephone ? `${indicatifPourPays(paysInscription)} ${telephone}` : "" } },
      });
      if (error) {
      ...
      // CORRECTIF — si la confirmation par email est activée côté
      // Supabase (Dashboard → Authentication → Settings), signUp() ne
      // crée PAS de session tant que le lien reçu par email n'est pas
      // cliqué : data.session est alors null. Avant ce correctif, le
      // code redirigeait quand même vers /securite/pin, une page
      // protégée par middleware.js qui exige une session — comme il
      // n'y en avait pas encore, l'utilisateur était systématiquement
      // renvoyé vers /auth, ce qui donnait l'impression que "ça ne
      // marche pas" (ou "ça prend trop de temps", en boucle).
      if (!data.session) {
        setEnCours(false);
        setSucces(true);
        setAttenteConfirmationEmail(true);
        return;
      }
  ...
  */
  const connexionSociale = async (fournisseur) => {
    ...
    // Passe par /auth/callback (côté serveur) pour échanger le code
    // OAuth contre une vraie session — voir app/auth/callback/route.js.
    // redirectTo = "/" directement ne fonctionnait pas : rien ne
    // consommait le code renvoyé par Google/Facebook/Apple.
    const suite = searchParams.get("suite") || "/";
    const { error } = await supabase.auth.signInWithOAuth({
      provider: fournisseur,
      options: { redirectTo: `${window.location.origin}/auth/callback?suite=${encodeURIComponent(suite)}` },
    });
    if (error) {
      setErreur(t("auth.compteImpossible"));
      ...
      <main className="max-w-md mx-auto w-full px-4 pt-24 pb-10 relative">
        {succes && (
          <div className="absolute inset-0 z-20 flex flex-col items-center justify-center p-6 tekca-pop" style={{ background: COLORS.background }}>
            <svg width="56" height="56" viewBox="0 0 52 52">
              ...
              <path d="M15 27l7 7 15-15" fill="none" stroke="#3A8A5C" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" strokeDasharray="48" style={{ animation: "tekcaCheckDraw 0.5s ease-out 0.15s both" }} />
            </svg>
            <p className="text-base font-bold mt-4">{mode === "connexion" ? t("auth.connexionReussie") : t("auth.compteCree")}</p>
            <p className="text-xs mt-1 text-center" style={{ color: COLORS.textMuted }}>
              {mode === "connexion"
                ? t("auth.redirection")
                : attenteConfirmationEmail
                ? t("auth.verifieBoiteMail")
                : t("auth.redirectionPin")}
            </p>
          </div>
        )}

        <p className="text-xl font-extrabold mb-1">
          {mode === "connexion" ? t("auth.contentDeTeRevoir") : t("auth.rejoinsEcosysteme")}
        </p>
        ...
        <div className="flex flex-col gap-3">
          ...
          >
            ...
            {enCoursSocial === "google" ? t("auth.unInstant") : t("auth.continuerGoogle")}
          </button>
          <button
          ...
          >
            ...
            {enCoursSocial === "facebook" ? t("auth.unInstant") : t("auth.continuerFacebook")}
          </button>
          <button
          ...
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill={theme === "sombre" ? "#111" : "#fff"}>
              <path d="M16.36 1c.15 1.05-.29 2.1-.94 2.86-.68.78-1.78 1.38-2.86 1.3-.16-1.02.34-2.08.98-2.79.71-.78 1.9-1.36 2.82-1.37zm3.36 6.14c-1.55-.09-2.87.87-3.61.87-.75 0-1.86-.85-3.06-.83-1.57.02-3.02.9-3.83 2.31-1.63 2.83-.42 7.02 1.18 9.32.78 1.13 1.71 2.4 2.94 2.35 1.17-.05 1.62-.76 3.03-.76s1.82.76 3.06.74c1.27-.02 2.07-1.14 2.85-2.28.9-1.29 1.27-2.55 1.28-2.62-.03-.01-2.45-.94-2.48-3.73-.02-2.33 1.9-3.45 1.99-3.5-1.09-1.6-2.78-1.78-3.35-1.87z" />
            </svg>
            {enCoursSocial === "apple" ? t("auth.unInstant") : t("auth.continuerApple")}
          </button>
          ...
          {/* CORRECTIF — le champ email n'était affiché qu'à
              l'inscription, jamais à la connexion, alors que
              seConnecter() (signInWithPassword) exige toujours
              l'email. Le téléphone, lui, n'est utile qu'à
              l'inscription (stocké en métadonnée) — jamais utilisé
              pour se connecter aujourd'hui. */}
          <div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <select
                value={paysInscription}
                onChange={(e) => setPaysInscription(e.target.value)}
                className="rounded-lg px-2 py-2 text-sm outline-none flex-shrink-0"
                style={{ background: COLORS.surface, color: COLORS.textPrimary, border: `1px solid ${COLORS.border}`, maxWidth: 132 }}
              >
                {PAYS_SEBPAY.map((p) => (
                  <option key={p.code} value={p.code}>{p.nom}</option>
                ))}
              </select>
              <div className="flex items-center gap-2 rounded-lg px-3 py-2 flex-1" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
                <span className="text-sm font-semibold flex-shrink-0" style={{ color: COLORS.accentSecondary }}>
                  {indicatifPourPays(paysInscription)}
                </span>
                <input
                  value={telephone}
          ...
          */}
          ...
          >
            ...
            <span className="text-xs" style={{ color: COLORS.textPrimary }}>{t("auth.pasUnRobot")}</span>
          </button>
