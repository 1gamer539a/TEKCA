...
// Clés de traduction (produits.*) au lieu de libellés en dur — voir
// leur usage plus bas avec t().
const CLE_TYPE = {
...
export default function ProfilVendeurMarche() {
  ...
  const charger = async () => {
    ...
    try {
      const { data: v, error: erreurVendeur } = await supabase
        ...
        .single();
      if (erreurVendeur) throw new Error(t("profilVendeur.vendeurIntrouvable"));
      setVendeur(v);
      ...
      setErreurChargement(e.message || t("profilVendeur.impossibleChargerProfilPoint"));
    } finally {
      setChargement(false);
    }
  };
  ...
  const typesDisponibles = ["tous", ...new Set(produits.map((p) => p.type))];
  const produitsFiltres = filtreType === "tous" ? produits : produits.filter((p) => p.type === filtreType);
  const libelleType = (cle) => (cle === "tous" ? t("common.tous") : (CLE_TYPE[cle] ? t(CLE_TYPE[cle]) : cle));
  ...
  return (
    <div style={{ background: COLORS.background, minHeight: "100vh", color: COLORS.textPrimary }}>
      ...
      >
        ...
        <span className="text-sm font-semibold">{t("profilVendeur.titre")}</span>
      ...
      <main className="max-w-md mx-auto w-full px-4 pt-20 pb-10">
        {chargement && (
          <div className="flex flex-col items-center justify-center gap-2 py-20">
            ...
            <p className="text-xs" style={{ color: COLORS.textMuted }}>{t("profilVendeur.chargementProfil")}</p>
          </div>
        )}
        ...
        {!chargement && erreurChargement && (
          <div className="flex flex-col items-center justify-center gap-3 py-20 text-center">
            ...
            <p className="text-sm font-semibold">{t("profilVendeur.impossibleChargerProfilTitre")}</p>
          ...
          <>
            ...
            <div className="flex items-center gap-3 mb-4">
              <div className="w-16 h-16 rounded-full flex items-center justify-center flex-shrink-0 relative" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
                ...
                <span
                  ...
                  title={vendeur.derniere_activite && (Date.now() - new Date(vendeur.derniere_activite).getTime()) < 5 * 60 * 1000 ? t("profilVendeur.enLigne") : t("profilVendeur.horsLigne")}
                />
              </div>
              <div>
                <div className="flex items-center gap-1.5">
                  ...
                  {vendeur.statut === "valide" && (
                    <span className="flex items-center gap-0.5 text-[10px] font-semibold px-1.5 py-0.5 rounded-full" style={{ background: COLORS.accentSecondary, color: COLORS.background }}>
                      <ShieldCheck size={10} /> {t("profilVendeur.verifie")}
                    </span>
                  )}
                </div>
                <p className="text-[11px] mt-0.5" style={{ color: vendeur.derniere_activite && (Date.now() - new Date(vendeur.derniere_activite).getTime()) < 5 * 60 * 1000 ? "#3A8A5C" : COLORS.textMuted }}>
                  {vendeur.derniere_activite && (Date.now() - new Date(vendeur.derniere_activite).getTime()) < 5 * 60 * 1000 ? t("profilVendeur.enLigne") : t("profilVendeur.horsLigne")}
                </p>
                ...
                <p className="text-[11px] flex items-center gap-1 mt-0.5" style={{ color: COLORS.textMuted }}>
                  <Calendar size={11} /> {t("profilVendeur.membreDepuis", { date: new Date(vendeur.date_creation).toLocaleDateString("fr-FR", { month: "long", year: "numeric" }) })}
                </p>
              </div>
            </div>
            ...
            {vendeur.en_pause && (
              <div className="rounded-lg px-3 py-2 mb-4 text-[11px]" style={{ background: "rgba(178,58,46,0.1)", color: "#B23A2E" }}>
                ⏸ {vendeur.message_pause || t("profilVendeur.indisponibleTemp")}
              </div>
            )}
            ...
            <div className="rounded-xl px-3 py-2 mb-4 text-[11px]" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, color: COLORS.textMuted }}>
              {t("profilVendeur.profilMarcheDesc")}
            </div>
            ...
            <button
              ...
              style={{ background: COLORS.accentPrimary, color: COLORS.background }}
            >
              <MessageCircle size={16} /> {ouvertureConversation ? t("dashboard.ouvertureEnCours") : t("profilVendeur.envoyerMessage")}
            </button>
            {erreurConversation && (
            ...
            {produits.length > 0 && (
              <div className="flex gap-2 mb-3 overflow-x-auto">
                {typesDisponibles.map((cle) => (
                  <button
                    key={cle}
                    onClick={() => setFiltreType(cle)}
                    className="text-xs px-3 py-1.5 rounded-full flex-shrink-0"
                    style={{
                      background: filtreType === cle ? COLORS.accentPrimary : COLORS.surface,
                      color: filtreType === cle ? COLORS.background : COLORS.textMuted,
            ...
            <p className="text-xs font-bold uppercase tracking-wide mb-2" style={{ color: COLORS.accentPrimary }}>
              {t("profilVendeur.sesAnnonces", { count: produitsFiltres.length })}
            </p>

            {produits.length === 0 && (
              <p className="text-sm text-center py-10" style={{ color: COLORS.textMuted }}>
                {t("profilVendeur.pasEncoreAnnonce")}
              </p>
            )}
