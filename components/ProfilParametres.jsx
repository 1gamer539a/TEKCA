  ...
  const basculerNotifs = async () => {
    ...
    if (notifsActives) {
      ...
      if (resultat.ok) {
        ...
        const messages = {
          non_supporte: "Ton navigateur ne supporte pas les notifications push.",
          refuse: "Autorisation refusée. Active les notifications pour TEKÇA dans les réglages de ton navigateur/téléphone pour réessayer.",
          cle_manquante: "Notifications push pas encore configurées côté serveur.",
          erreur_serveur: "Erreur lors de l'activation, réessaie.",
        };
        setNotifsErreur(messages[resultat.raison] || "Impossible d'activer les notifications.");
      }
    }
        ...
        <div className="rounded-xl p-3 mb-6" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
          ...
          {!pushSupporte() && !notifsErreur && (
            <p className="text-[11px] mt-2" style={{ color: COLORS.textMuted }}>
              Sur iPhone, ajoute d'abord TEKÇA à ton écran d'accueil pour pouvoir activer les notifications.
            </p>
          )}
        </div>
