...
export default function ProfilParametres() {
  ...
  const [estVendeurValide, setEstVendeurValide] = useState(false);
  ...
  useEffect(() => {
    const chargerProfil = async () => {
      ...
      // Lu directement depuis "vendeurs" plutôt que "users.role" — ce
      // dernier n'est qu'une copie mise à jour manuellement à chaque
      // changement de statut (voir DevenirVendeur.jsx et
      // /api/admin/vendeur-statut) et peut se désynchroniser pour les
      // comptes créés avant l'ajout de cette synchro. "vendeurs" reste
      // la seule source de vérité, donc on l'interroge ici aussi pour
      // ne plus jamais dépendre de cette copie.
      const { data: vendeurValide } = await supabase
        .from("vendeurs")
        .select("id")
        .eq("user_id", user.id)
        .eq("statut", "valide")
        .maybeSingle();
      setEstVendeurValide(!!vendeurValide);
  ...
  const SECTIONS = [
    {
      titre: "Compte",
      items: [
        { label: "Informations personnelles", icon: User, href: "/compte/informations" },
        { label: "Vérifier mon identité", icon: ShieldCheck, href: "/verification-identite" },
        { label: "Mes réclamations", icon: MessageCircle, href: "/reclamations" },
        { label: "Mon portefeuille", icon: Wallet, href: "/portefeuille" },
        { label: "Mes commandes", icon: Store, href: "/commandes" },
        { label: "Mes achats numériques", icon: KeyRound, href: "/achats-numeriques" },
    ...
    },
    {
      titre: "Sécurité",
      items: [
        { label: "Modifier mon code PIN", icon: Lock, href: "/securite/modifier-pin" },
      ],
    },
    {
      titre: "Support",
      items: [
        ...
        { label: "Politique de livraison & retours", icon: ShieldCheck, href: "/politique-livraison" },
      ],
    },
  ];
