...
function FicheCoffreFort({ COLORS, produit }) {
  ...
  const acheter = async () => {
    ...
    try {
      ...
      if (!reponse.ok) {
        setErreur(data.error || "Erreur lors de l'achat.");
        return;
      }
      setAchatReussi(true);
    } catch {
  ...
  if (achatReussi) {
    return (
      <div className="max-w-md mx-auto w-full px-4 pt-20 pb-10 flex flex-col items-center text-center">
        ...
        <p className="font-bold text-lg" style={{ color: COLORS.textPrimary }}>Achat confirmé</p>
        <p className="text-sm mt-2 max-w-xs" style={{ color: COLORS.textMuted }}>
          Ton code pour "{produit.nom}" est prêt. Pour le voir, va dans "Mes achats numériques" et entre ton code PIN.
        </p>
        <button
  ...
  return (
    <div className="max-w-md mx-auto w-full px-4 pt-20 pb-10">
      ...
      >
        {achatEnCours ? "Traitement..." : stock === 0 ? "Épuisé" : "Acheter maintenant"}
      </button>
    </div>
  );
}
    ...
    <div className="max-w-md mx-auto w-full px-4 pt-20 pb-10">
      ...
      {produit.modesRemise?.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          {produit.modesRemise.includes("instantane") && (
            <span className="text-[11px] px-2.5 py-1 rounded-full flex items-center gap-1" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, color: COLORS.textMuted }}>
              ⚡ Livraison instantanée / Digitale
            </span>
          )}
        </div>
      )}
      {produit.zoneCouverture && (
        <p className="text-[11px] mb-4 -mt-2" style={{ color: COLORS.textMuted }}>
          Livré dans : {produit.zoneCouverture}
        </p>
      )}
      {produit.description && (
        <div className="rounded-2xl p-4 mb-4" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
