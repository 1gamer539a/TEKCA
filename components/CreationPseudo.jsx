...
export default function CreationPseudo() {
  ...
  const creer = async () => {
    setErreur(null);
    if (pseudo.length < 3) {
      setErreur(t("pseudo.troisCarMin"));
      return;
    }
    if (!dateNaissance) {
      setErreur(t("pseudo.renseigneDateNaissance"));
      return;
    }
    setEnCours(true);
    try {
      ...
      const reponse = await fetch("/api/identite/creer-pseudo", {
        ...
        body: JSON.stringify({ pseudo, dateNaissance }),
      });
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6" style={{ background: COLORS.background }}>
        ...
        <div className="w-full max-w-xs rounded-2xl p-5 mt-6 text-center" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}` }}>
          <p className="text-xs uppercase tracking-wide" style={{ color: COLORS.textMuted }}>{t("pseudo.pseudoLabel")}</p>
          <p className="text-lg font-bold" style={{ color: COLORS.textPrimary }}>@{resultat.pseudo}</p>
          <div className="mt-3 pt-3" style={{ borderTop: `1px solid ${COLORS.border}` }}>
            <p className="text-xs uppercase tracking-wide" style={{ color: COLORS.textMuted }}>{t("pseudo.identifiantTekca")}</p>
            <div className="flex items-center justify-center gap-2 mt-1">
              <p className="text-2xl font-extrabold tracking-wider" style={{ color: COLORS.accentPrimary }}>{resultat.identifiant}</p>
              <button onClick={copier} aria-label={t("pseudo.copier")}>
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6" style={{ background: COLORS.background }}>
      <h1 className="text-xl font-bold text-center" style={{ color: COLORS.textPrimary }}>
        {t("pseudo.choisisPseudo")}
      </h1>
      <p className="text-sm text-center mt-2" style={{ color: COLORS.textMuted }}>
        {t("pseudo.pseudoServiraDesc")}
      </p>
      ...
      >
        ...
        <span>{t("pseudo.pseudoDefinitifDesc")}</span>
      </div>
      ...
      />
      <p className="text-[11px] mt-1" style={{ color: COLORS.textMuted }}>{t("pseudo.pseudoCaracteresDesc")}</p>

      <label className="w-full max-w-xs text-left text-xs font-semibold mt-4 mb-1" style={{ color: COLORS.textPrimary }}>
        {t("pseudo.dateNaissanceLabel")}
      </label>
      <input
        type="date"
        value={dateNaissance}
        ...
        className="w-full max-w-xs rounded-xl px-4 py-3 text-sm outline-none"
        style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, color: COLORS.textPrimary }}
      />
      <p className="text-[11px] mt-1" style={{ color: COLORS.textMuted }}>{t("pseudo.accessibleDes14ans")}</p>

      {erreur && <p className="text-xs mt-3 text-center" style={{ color: "#B23A2E" }}>{erreur}</p>}

      <button
        onClick={creer}
        disabled={enCours || pseudo.length < 3 || !dateNaissance}
        className="w-full max-w-xs rounded-xl py-3 font-semibold mt-5"
        style={{
          background: pseudo.length >= 3 && dateNaissance ? COLORS.accentPrimary : COLORS.border,
          color: pseudo.length >= 3 && dateNaissance ? COLORS.background : COLORS.textMuted,
        }}
      >
        {enCours ? t("pseudo.creationDots") : t("pseudo.validerDefinitivement")}
      </button>
    </div>
  );
}
