...
export default function MesReclamations() {
  ...
  const LABELS_STATUT = {
    ouvert: { label: t("reclamations.statutOuvert"), color: "#B23A2E" },
    en_cours: { label: t("reclamations.statutEnCours"), color: "#C99A3A" },
    resolu: { label: t("reclamations.statutResolu"), color: "#3A8A5C" },
  };
  ...
  return (
    <div style={{ background: COLORS.background, minHeight: "100vh", color: COLORS.textPrimary }}>
      ...
      >
        ...
        <span className="text-sm font-semibold">{t("reclamations.titre")}</span>
      ...
      <main className="max-w-md mx-auto w-full px-4 pt-20 pb-10">
        {chargement && <p className="text-xs text-center py-10" style={{ color: COLORS.textMuted }}>{t("common.chargement")}</p>}
        {!chargement && reclamations.length === 0 && (
          <p className="text-sm text-center py-10" style={{ color: COLORS.textMuted }}>{t("reclamations.aucuneReclamation")}</p>
        )}
        {reclamations.map((r) => {
          ...
          return (
            ...
            >
              ...
              <p className="text-[10.5px] mt-1.5 flex items-center gap-1" style={{ color: COLORS.accentPrimary }}>
                <MessageCircle size={11} /> {t("reclamations.voirLeFil")}
              </p>
            </Link>
          );
        })}
