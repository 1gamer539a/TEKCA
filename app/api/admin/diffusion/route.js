...
/*
  ...
  utilisateurs, ET comme vraie notification système (voir
  lib/push-serveur.js, migration_push_subscriptions.sql) pour ceux
  qui ont activé les notifications — via notifierPlusieurs, qui gère
...
export async function POST(req) {
  try {
    ...
    if (utilisateurs?.length) {
      ...
      const { error: erreurNotifs } = await notifierPlusieurs(supabaseAdmin, lignes);
