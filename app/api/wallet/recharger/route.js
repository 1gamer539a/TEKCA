...
import { initierRecharge, urlWebhookSebpay } from "../../../../lib/sebpay";
...
export async function POST(req) {
  try {
    ...
    if (montantNum < SEUIL_DEPOT_MIN) {
      return NextResponse.json({ error: `Le montant minimum pour une recharge est de ${SEUIL_DEPOT_MIN.toLocaleString()} FCFA.` }, { status: 400 });
    }
    if (!operateur) {
    ...
    return NextResponse.json({
      ...
      // provider_link n'est présent que pour certains opérateurs (ex: Wave) —
      // quand il est là, le frontend DOIT rediriger l'utilisateur vers cette
      // URL dans un nouvel onglet pour qu'il valide le paiement (doc SebPay).
      // Pour les autres opérateurs (MTN, Airtel...), le client reçoit un
      // push USSD directement sur son téléphone et ce champ est absent.
      lienPaiement: resultatSeePay.provider_link || null,
      statutInitial: resultatSeePay.status,
    });
  } catch (e) {
