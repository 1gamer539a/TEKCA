...
export async function POST(req) {
  try {
    ...
    // Vérification du solde — jamais confiée au client, toujours recalculée ici.
    ...
    // Débite immédiatement le wallet (évite un double-retrait en cas de
    // double-clic pendant l'appel réseau à SeePay)
    await supabaseAdmin
      .from("wallets")
      .update({ solde: soldeActuel - montantNum, date_maj: new Date().toISOString() })
      .eq("user_id", user.id);
    ...
    try {
      ...
      // Echec SeePay : on recrédite le wallet et on marque la transaction échouée
      await supabaseAdmin
        .from("wallets")
        .update({ solde: soldeActuel, date_maj: new Date().toISOString() })
        .eq("user_id", user.id);
