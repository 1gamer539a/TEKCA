...
export async function POST(req) {
  try {
    ...
    return poserCookiePinVerifie(NextResponse.json({ ok: true }));
  } catch (e) {
...
/*
  CORRECTIF — le PIN n'était jamais redemandé après une nouvelle
  connexion (le middleware ne vérifiait que pseudo/date de naissance,
  pas le PIN) : on pose ici un cookie httpOnly de courte durée qui
  atteste "PIN vérifié pour cette session", que middleware.js peut
  lire sans base de données à chaque requête. httpOnly + pas
  d'accès en JS : seul ce serveur peut le poser ou le retirer, un
  script côté client ne peut pas le fabriquer pour contourner le
  contrôle. Pas de maxAge fixe long : la valeur "session" (pas de
  maxAge) fait que le cookie disparaît à la fermeture du navigateur ;
  il est aussi retiré explicitement à la déconnexion (voir
  /api/securite/pin/deconnecter).
*/
function poserCookiePinVerifie(reponse) {
  reponse.cookies.set("tekca_pin_ok", "1", {
    httpOnly: true,
