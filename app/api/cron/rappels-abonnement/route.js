...
export async function GET(req) {
  try {
    ...
    // Cette requête ne sert à rien de métier — un simple count sur
    // une table légère. Son seul rôle est de compter comme "activité"
    // aux yeux de Supabase, qui met en pause tout projet gratuit
    // resté 7 jours sans requête. Regroupée ici (plutôt qu'un 3e
    // cron séparé) pour rester dans la limite de crons du plan
    // Vercel Hobby.
    ...
    for (const abonnement of abonnementsAExpirer || []) {
      ...
      await notifier(supabaseAdmin, { user_id: abonnement.user_id, type: "message", texte });
