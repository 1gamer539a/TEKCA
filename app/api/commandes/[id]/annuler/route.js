...
export async function POST(req, { params }) {
  try {
    ...
    const { data: commande } = await supabaseAdmin
      .from("commandes")
      .select("id, client_id, vendeur_id, produit_id, variante_id, quantite, montant_total, statut")
      .eq("id", commandeId)
      .single();
    ...
    // Restitue le stock consommé à la commande, si applicable —
    // sinon un article annulé resterait décompté du stock pour
    // toujours (voir migration_stock_atomique.sql).
    if (commande.variante_id) {
      ...
      const { data: produitCommande } = await supabaseAdmin.from("produits").select("stock_global").eq("id", commande.produit_id).single();
      if (produitCommande?.stock_global !== null && produitCommande?.stock_global !== undefined) {
